...
/* ════════════════════════════════════════
   RECONSTRUCTED WATERCOLOR TEDDY
   Refăcut exact după modelul primit în imagine
════════════════════════════════════════ */
  ...
  return (
    <svg viewBox="0 0 240 380" fill="none" xmlns="http://www.w3.org/2000/svg"
      style={{width:'100%',height:'auto',display:'block',overflow:'visible',
        animation: wiggle ? 'tWiggle .3s ease-in-out infinite alternate' : 'tFloat 3.8s ease-in-out infinite'}}>
      <defs>
        <radialGradient id="t_head" cx="45%" cy="35%" r="60%">
      ...
      {/* ══ BALLOON (Spate și Sfoară) ══ */}
      {/* Sfoara fină și fluidă */}
      <path d="M142 144 C155 190 145 250 152 305" stroke="#B2A494" strokeWidth="1.2" fill="none" strokeLinecap="round"/>
      
      {/* Nodul balonului */}
      <path d="M136 142 L142 152 L148 142 Z" fill="#A2CBE6" filter="url(#watercolor_effect)"/>
      <path d="M134 145 C138 141 146 141 150 145" stroke="#8AB1CC" strokeWidth="1" fill="none"/>

      {/* Corpul Balonului */}
      <circle cx="142" cy="74" r="50" fill="url(#t_balloon)" filter="url(#watercolor_effect)"/>
      
      {/* Decor interior balon (Stele & Puncte) */}
      <g opacity="0.45" fill="#FFFFFF">
        {/* Steaua mare centrală */}
        <path d="M142 54 L144 60 L150 60 L145 64 L147 70 L142 66 L137 70 L139 64 L134 60 L140 60 Z"/>
        {/* Stele mici */}
        <path d="M166 84 L167 87 L170 87 L168 89 L169 92 L166 90 L163 92 L164 89 L162 87 L165 87 Z" transform="scale(0.7) translate(60, 20)"/>
        <path d="M115 90 L116 93 L119 93 L117 95 L118 98 L115 96 L112 98 L113 95 L111 93 L114 93 Z" transform="scale(0.6) translate(65, 50)"/>
        {/* Buline/Inimi soft */}
      ...
      {/* Strălucire/Highlight Balon */}
      <ellipse cx="124" cy="56" rx="14" ry="24" fill="#FFFFFF" opacity="0.3" transform="rotate(-30 124 56)"/>

      {/* Funda Balonului */}
      <g filter="url(#watercolor_effect)" stroke="#8AB1CC" strokeWidth="1" fill="none">
        <path d="M142 144 C132 144 126 132 136 135 C142 137 142 144 142 144 Z" fill="#C2DCED" opacity="0.8"/>
        <path d="M142 144 C152 144 158 132 148 135 C142 137 142 144 142 144 Z" fill="#C2DCED" opacity="0.8"/>
      </g>


      {/* ══ URECHI ══ */}
      <g filter="url(#watercolor_effect)">
        {/* Stânga */}
        <circle cx="64" cy="164" r="24" fill="url(#t_head)"/>
        <ellipse cx="64" cy="164" rx="14" ry="12" fill="#E2CDAF" opacity="0.7"/>
        {/* Dreapta */}
        <circle cx="156" cy="160" r="24" fill="url(#t_head)"/>
        <ellipse cx="156" cy="160" rx="14" ry="12" fill="#E2CDAF" opacity="0.7"/>
      </g>


      {/* ══ CORP ══ */}
      <circle cx="112" cy="274" r="54" fill="url(#t_body)" filter="url(#watercolor_effect)"/>


      {/* ══ PICIOARE ══ */}
      <g filter="url(#watercolor_effect)">
        {/* Stânga */}
        <ellipse cx="88" cy="336" rx="22" ry="24" fill="url(#t_body)"/>
        <ellipse cx="88" cy="340" rx="15" ry="16" fill="#EAD9C2" opacity="0.6"/>
        {/* Dreapta */}
        <ellipse cx="140" cy="334" rx="22" ry="24" fill="url(#t_body)"/>
        <ellipse cx="140" cy="340" rx="15" ry="16" fill="#EAD9C2" opacity="0.6"/>
      </g>


      {/* ══ MÂNA STÂNGA (Jos) ══ */}
      <ellipse cx="60" cy="264" rx="16" ry="22" fill="url(#t_body)" filter="url(#watercolor_effect)" transform="rotate(20 60 264)"/>


      {/* ══ CAP ══ */}
      <ellipse cx="110" cy="214" rx="60" ry="54" fill="url(#t_head)" filter="url(#watercolor_effect)"/>


      {/* ══ BOT (Muzzle) ══ */}
      <ellipse cx="112" cy="234" rx="26" ry="20" fill="url(#t_muzzle)" filter="url(#watercolor_effect)"/>


      {/* ══ DETALII FAȚĂ (Ochi maro cald, fără gene stridente) ══ */}
      {/* Ochiul Stâng */}
      ...
      {/* Guriță/Zâmbet dulce */}
      <path d="M105 231 Q111 236 111 231 Q111 236 117 231" stroke="#422A1D" strokeWidth="1.8" fill="none" strokeLinecap="round"/>

      {/* Blush Obraji delicat acuarelă */}
      <ellipse cx="76" cy="228" rx="10" ry="6" fill="#F3A395" opacity="0.32" filter="url(#watercolor_effect)"/>
      <ellipse cx="142" cy="226" rx="10" ry="6" fill="#F3A395" opacity="0.32" filter="url(#watercolor_effect)"/>


      {/* ══ MÂNA DREAPTĂ (Ridicată, ține sfoara) ══ */}
      <ellipse cx="146" cy="246" rx="15" ry="20" fill="url(#t_body)" filter="url(#watercolor_effect)" transform="rotate(-35 146 246)"/>


      {/* ══ FUNDĂ MARE LA GÂT (Bleu cu buline) ══ */}
      <g filter="url(#watercolor_effect)">
        {/* Aripa stângă */}
        <path d="M110 256 C88 244 80 266 94 274 C104 280 108 266 110 256 Z" fill="url(#t_bow)"/>
        {/* Aripa dreaptă */}
        <path d="M114 256 C136 244 144 266 130 274 C120 280 116 266 114 256 Z" fill="url(#t_bow)"/>
        {/* Nodul central */}
        <ellipse cx="112" cy="259" rx="8" ry="7" fill="#7DAFD0"/>
      </g>
      {/* Buline albe pe fundă */}
      ...
      {/* Elemente de fundal soft din imagine (Inimi & Stele rătăcite) */}
      <g opacity="0.25">
        <path d="M34 260 C34 257 31 255 29 257 C27 259 29 263 34 266 C39 263 41 259 39 257 C37 255 34 257 34 260Z" fill="#90BEDB"/>
        <path d="M184 220 C184 217 181 215 179 217 C177 219 179 223 184 226 C189 223 191 219 189 217 C187 215 184 217 184 220Z" fill="#90BEDB"/>
        <path d="M30 180 L31 183 L34 183 L32 185 L33 188 L30 186 L27 188 L28 185 L26 183 L29 183 Z" fill="#EAD8BA"/>
      </g>
    </svg>
  )
}
...
function BgDecor() {
  return (
    <div style={{position:'fixed',inset:0,overflow:'hidden',pointerEvents:'none',zIndex:1}}>
      ...
      {BALLOONS_BG.map((b,i)=>(
        <div key={i} style={{position:'absolute',bottom:'-20%',left:b.l,width:'clamp(30px,5.5vw,50px)',opacity:.38,animation:`riseUp ${b.d}s linear infinite ${b.delay}s`}}>
          <BalloonBG c={b.c}/>
        </div>
      ))}
      <div style={{position:'absolute',top:'-8%',right:'-6%',width:'clamp(180px,32vw,360px)',aspectRatio:'1',borderRadius:'50%',background:'radial-gradient(circle,rgba(200,165,110,.12),transparent 70%)',animation:'breathe 9s ease-in-out infinite'}}/>
      <div style={{position:'absolute',bottom:'-10%',left:'-6%',width:'clamp(150px,26vw,300px)',aspectRatio:'1',borderRadius:'50%',background:'radial-gradient(circle,rgba(152,192,220,.12),transparent 70%)',animation:'breathe 12s ease-in-out infinite 2s'}}/>
    </div>
  )
}
  ...
  return (
    <div style={{position:'fixed',inset:0,top:56,overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      <div style={{position:'relative',zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',gap:10,padding:'16px 22px',width:'100%',maxWidth:480}}>
        <p style={{fontFamily:"'Quicksand',sans-serif",fontSize:'clamp(9px,1.1vw,11px)',letterSpacing:'.32em',textTransform:'uppercase',color:'#7AAAC0',fontWeight:700,opacity:bouncing?0:1,transition:'opacity .4s',animation:'fadeUp .7s ease both'}}>Invitație la Botez</p>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(34px,7vw,66px)',fontWeight:500,fontStyle:'italic',color:'#8B5E3C',textAlign:'center',lineHeight:1.05,margin:0,opacity:bouncing?0:1,transition:'opacity .4s',textShadow:'0 3px 20px rgba(139,94,60,.14)',animation:'fadeUp .8s ease both .08s'}}>
          Matei Alexandru
        </h1>
        <p style={{fontFamily:"'Cormorant',serif",fontSize:'clamp(14px,2vw,18px)',fontStyle:'italic',color:'#A07850',textAlign:'center',lineHeight:1.5,opacity:bouncing?0:1,transition:'opacity .4s',animation:'fadeUp .9s ease both .16s'}}>
        ...
        <div onClick={onStart} role="button" tabIndex={0} onKeyDown={e=>e.key==='Enter'&&onStart()}
          ...
          {bouncing && (
            <div style={{position:'absolute',inset:0,pointerEvents:'none',animation:'fadeOut 1.8s ease forwards'}}>
              {['✨','🎈','💙','⭐'].map((e,i)=>(
                <span key={i} style={{position:'absolute',fontSize:20,left:`${12+i*20}%`,top:`${20+(i%3)*18}%`,animation:`burstUp .9s ease ${i*.12}s both`}}>{e}</span>
              ))}
            </div>
          )}
        </div>
        <p style={{fontFamily:"'Quicksand',sans-serif",fontSize:'clamp(9px,1.1vw,11px)',letterSpacing:'.24em',textTransform:'uppercase',color:'#7AAAC0',fontWeight:700,animation:bouncing?'none':'fadeUp 1s ease both .4s, pulse 2.8s ease-in-out infinite 1.4s'}}>
...
/* ── INVITE ── */
function InviteScreen({ onBack:_ }: { onBack:()=>void }) {
  const BAPTISM = new Date('2027-10-09T12:00:00')
  ...
  const mH = (q: string) => `https://google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`
  const wH = (q: string) => `https://waze.com/ul?q=${encodeURIComponent(q)}&navigate=yes`
  ...
  const events = [
    {emoji:'⛪',type:'Taina Botezului',venue:'Catedrala Adormirea Maicii Domnului',addr:'Str. Mitropolit Veniamin 2, Iași',time:'12:00',q:'Catedrala Adormirea Maicii Domnului, Str. Mitropolit Veniamin 2, Iași'},
    {emoji:'🎉',type:'Recepție',venue:'Restaurant La Maison',addr:'Str. Elena Doamna 54, Iași',time:'15:30',q:'Restaurant La Maison, Str. Elena Doamna 54, Iași'},
  ]
      ...
      <div style={{position:'relative',zIndex:2,maxWidth:860,margin:'0 auto',padding:'32px 18px 56px'}}>
        ...
        <div style={{...a(0),display:'flex',alignItems:'center',gap:'clamp(14px,4vw,44px)',marginBottom:24,flexWrap:'wrap',justifyContent:'center'}}>
          ...
          <div style={{flex:'1 1 240px',textAlign:'left'}}>
            ...
            <span style={{display:'block',fontFamily:"'Playfair Display',serif",fontSize:'clamp(42px,8vw,90px)',fontWeight:500,fontStyle:'italic',color:'#8B5E3C',lineHeight:.95,textShadow:'0 4px 24px rgba(139,94,60,.12)'}}>Matei<br/>Alexandru</span>
            <span style={{display:'block',fontFamily:"'Cormorant',serif",fontSize:'clamp(14px,1.8vw,18px)',fontStyle:'italic',color:'#A07850',marginTop:10,lineHeight:1.5}}>Fiul lui Laura & Dan<br/><span style={{fontSize:'clamp(11px,1.3vw,14px)',letterSpacing:'.05em'}}>9 Octombrie 2027 · Iași</span></span>
          </div>
        </div>
          ...
          <div style={{display:'flex',justifyContent:'center'}}>
            {[{n:pad(cd.d),l:'Zile'},{n:pad(cd.h),l:'Ore'},{n:pad(cd.m),l:'Minute'},{n:pad(cd.s),l:'Secunde',flip:flipS}].map((u,i)=>(
              <div key={u.l} style={{flex:1,maxWidth:110,textAlign:'center',padding:'0 6px',borderRight:i<3?'1px solid rgba(122,170,192,.18)':undefined}}>
                <span style={{display:'block',fontFamily:"'Cormorant',serif",fontSize:'clamp(34px,5.5vw,56px)',fontWeight:300,lineHeight:1,color:(u as {flip?:boolean}).flip?'#7AAAC0':'#8B5E3C',transition:'all .15s',transform:(u as {flip?:boolean}).flip?'scale(1.12) translateY(-3px)':'scale(1)'}}>{u.n}</span>
        ...
        <div style={{...a(.2),display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:'clamp(8px,2vw,20px)',alignItems:'center',marginBottom:20}}>
          {[{emoji:'🧸',label:'Părinții',name:'Laura & Dan'},{emoji:'🎈',label:'Nașii',name:'Ioana & Radu'}].map((f,idx)=>(
            <>
              <div key={idx} style={{textAlign:'center',...glass,padding:'16px 12px'}}>
              ...
              {idx===0 && (
                <div key="sep" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:5}}>
              ...
              <div style={{padding:'14px 16px',flex:1}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:8}}>
                  <div>
                    ...
                    <p style={{fontSize:'clamp(10px,1.1vw,12px)',color:'#A07850',lineHeight:1.5}}>{ev.addr}</p>
                  </div>
        ...
        <div style={{...a(.4),display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:14,marginBottom:20}}>
          <div style={{...glass,padding:'16px 18px'}}>
            ...
            <p style={{fontFamily:"'Quicksand',sans-serif",fontSize:13,color:'#5090B0',letterSpacing:'.06em',fontWeight:700,marginBottom:12}}>0745 321 678</p>
            <div style={{display:'flex',gap:8}}>
              <a href="tel:0745321678" style={{flex:1,display:'inline-flex',alignItems:'center',justifyContent:'center',gap:5,padding:'9px 6px',borderRadius:11,background:'linear-gradient(135deg,#7AAAC0,#4880A0)',color:'#fff',fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,textDecoration:'none'}}><PhIcon/> Telefon</a>
              <a href="https://wa.me/40745321678" target="_blank" rel="noopener noreferrer" style={{flex:1,display:'inline-flex',alignItems:'center',justifyContent:'center',gap:5,padding:'9px 6px',borderRadius:11,background:'linear-gradient(135deg,#25D366,#1DA851)',color:'#fff',fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,textDecoration:'none'}}><WAIcon/> WhatsApp</a>
            </div>
          </div>
          <div style={{...glass,padding:'16px 18px',display:'flex',flexDirection:'column',justifyContent:'space-between',gap:10}}>
            <div>
              ...
              <p style={{fontFamily:"'Cormorant',serif",fontSize:'clamp(13px,1.5vw,16px)',fontStyle:'italic',color:'#A07850',lineHeight:1.6}}>Vă rugăm să confirmați<br/>până pe <strong style={{color:'#8B5E3C',fontStyle:'normal'}}>25 septembrie 2027</strong></p>
            </div>
            <button onClick={()=>setModal(true)} style={{width:'100%',padding:'14px 0',borderRadius:100,background:'linear-gradient(135deg,#7AAAC0 0%,#4880A0 100%)',color:'#fff',fontFamily:"'Quicksand',sans-serif",fontSize:12,fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',cursor:'pointer',border:'none',boxShadow:'0 10px 30px rgba(72,128,160,.35)',transition:'transform .22s,box-shadow .22s',position:'relative',overflow:'hidden'} as React.CSSProperties}
        ...
        <div style={{...a(.58),...glass,padding:'18px 22px 22px',display:'flex',flexDirection:'column',alignItems:'center',gap:11}}>
          ...
          <p style={{fontFamily:"'Quicksand',sans-serif",fontSize:9,letterSpacing:'.2em',textTransform:'uppercase',color:'rgba(122,170,192,.55)',fontWeight:700,margin:0}}>VibeInvite © 2026 · Toate drepturile rezervate</p>
        </div>
      </div>

      {/* MODAL */}
      {modal && (
        <div onClick={()=>setModal(false)} style={{position:'fixed',inset:0,zIndex:300,background:'rgba(60,80,100,.38)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,animation:'fadeIn .28s ease',overflowY:'auto'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'linear-gradient(165deg,#FEFCF7,#EEF6FF)',borderRadius:28,padding:'clamp(22px,4vw,36px) clamp(16px,4vw,30px)',maxWidth:450,width:'100%',border:'1px solid rgba(122,170,192,.26)',boxShadow:'0 40px 100px rgba(60,80,100,.22)',animation:'slideUp .32s cubic-bezier(.4,0,.2,1)',maxHeight:'90vh',overflowY:'auto'}}>
            <div style={{textAlign:'center',marginBottom:20}}>
              <div style={{width:75,margin:'0 auto 10px'}}><TeddyWatercolor/></div>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(20px,3vw,26px)',fontStyle:'italic',fontWeight:500,color:'#8B5E3C',marginBottom:5}}>Confirmă Participarea</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              <div>
                <label style={{display:'block',fontFamily:"'Quicksand',sans-serif",fontSize:10,letterSpacing:'.16em',textTransform:'uppercase',color:'#7AAAC0',fontWeight:700,marginBottom:7}}>Nume și Prenume</label>
                <input type="text" placeholder="ex. Maria Popescu" style={{width:'100%',padding:'10px 13px',borderRadius:11,border:'1.5px solid rgba(122,170,192,.28)',background:'rgba(255,255,255,.88)',fontFamily:"'Nunito',sans-serif",fontSize:13,color:'#8B5E3C',outline:'none'}}
                  onFocus={e=>(e.currentTarget as HTMLInputElement).style.borderColor='rgba(122,170,192,.68)'}
                  onBlur={e=>(e.currentTarget as HTMLInputElement).style.borderColor='rgba(122,170,192,.28)'}/>
              </div>
              <div>
                <label style={{display:'block',fontFamily:"'Quicksand',sans-serif",fontSize:10,letterSpacing:'.16em',textTransform:'uppercase',color:'#7AAAC0',fontWeight:700,marginBottom:7}}>Veți fi însoțit/ă?</label>
                <div style={{display:'flex',gap:8}}>
                  {['Da','Nu'].map(o=><div key={o} onClick={()=>setIns(o)} style={chip(ins===o)}>{o}</div>)}
                </div>
              </div>
              <div>
                <label style={{display:'block',fontFamily:"'Quicksand',sans-serif",fontSize:10,letterSpacing:'.16em',textTransform:'uppercase',color:'#7AAAC0',fontWeight:700,marginBottom:7}}>Număr persoane</label>
                <div style={{display:'flex',gap:8}}>
                  {['1','2','3','4','5+'].map(o=><div key={o} onClick={()=>setPers(o)} style={chip(pers===o)}>{o}</div>)}
                </div>
              </div>
            </div>
            <div style={{marginTop:20,textAlign:'center'}}>
              <button onClick={()=>{setModal(false);setConfetti(true);setTimeout(()=>setConfetti(false),3500)}} style={{display:'block',width:'100%',padding:'13px 0',borderRadius:100,background:'linear-gradient(135deg,#7AAAC0,#4880A0)',color:'#fff',fontFamily:"'Quicksand',sans-serif",fontSize:12,fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',border:'none',cursor:'pointer',boxShadow:'0 8px 26 rgba(72,128,160,.32)',transition:'transform .2s'} as React.CSSProperties}
                onMouseEnter={e=>(e.currentTarget as HTMLButtonElement).style.transform='translateY(-2px)'}
                onMouseLeave={e=>(e.currentTarget as HTMLButtonElement).style.transform=''}>
                🎈 Confirmă Participarea
              </button>
              <div style={{marginTop:14,padding:'13px 15px',background:'rgba(152,192,220,.1)',border:'1px solid rgba(122,170,192,.2)',borderRadius:11}}>
                <p style={{fontFamily:"'Cormorant',serif",fontSize:13,fontStyle:'italic',color:'#A07850',lineHeight:1.8}}>
                  Mulțumim! 🧸<br/>Aceasta este o demonstrație a temei <strong style={{color:'#4880A0',fontStyle:'normal'}}>Ursulet</strong>.<br/>Achiziționează pachetul pentru a activa confirmările.
                </p>
              </div>
              <button onClick={()=>setModal(false)} style={{marginTop:9,background:'none',border:'none',cursor:'pointer',fontFamily:"'Cormorant',serif",fontSize:13,fontStyle:'italic',color:'rgba(122,170,192,.7)',textDecoration:'underline'}}>Închide</button>
            </div>
          </div>
        </div>
      )}
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes tFloat{0%,100%{transform:translateY(0) rotate(-.5deg)}50%{transform:translateY(-10px) rotate(.5deg)}}
        @keyframes tWiggle{from{transform:rotate(-4deg) scale(1.02)}to{transform:rotate(4deg) scale(.98)}}
        @keyframes riseUp{0%{transform:translateY(0) rotate(-2deg);opacity:0}10%{opacity:.4}85%{opacity:.4}100%{transform:translateY(-115vh) rotate(2deg);opacity:0}}
        @keyframes breathe{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
        @keyframes twinkle{0%,100%{opacity:.22;transform:scale(.78) rotate(0deg)}50%{opacity:.85;transform:scale(1.18) rotate(14deg)}}
        @keyframes burstUp{0%{opacity:0;transform:scale(.2)}60%{opacity:1}100%{opacity:0;transform:scale(1) translateY(-55px)}}
        @keyframes fadeOut{0%,72%{opacity:1}100%{opacity:0}}
        @keyframes shimmer{0%{background-position:-350px 0}100%{background-position:350px 0}}
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        @keyframes slideUp{from{opacity:0;transform:scale(.93) translateY(16px)}to{opacity:1;transform:scale(1) translateY(0)}}
        @keyframes pulse{0%,100%{opacity:.44}50%{opacity:.95}}
        @media(max-width:480px){body{overflow:hidden}}
      `}</style>

      <header style={{position:'fixed',top:0,left:0,right:0,zIndex:200,height:56,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 24px',background:'rgba(254,252,247,.93)',borderBottom:'1px solid rgba(122,170,192,.16)',backdropFilter:'blur(14px)'}}>
        <a href="/invitatii-digitale" style={{fontFamily:"'Quicksand',sans-serif",fontSize:12,fontWeight:700,letterSpacing:'.22em',textTransform:'uppercase',color:'#8B5E3C',textDecoration:'none',transition:'color .2s'}}
        ...
        <div style={{fontFamily:"'Cormorant',serif",fontSize:16,fontStyle:'italic',color:'#A07850',letterSpacing:'.04em'}}>
          {phase==='invite'?'Matei Alexandru · 9 Oct 2027':'Invitație la Botez'}
        </div>
        <a href="/invitatii-digitale" style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:100,background:'rgba(122,170,192,.12)',border:'1px solid rgba(122,170,192,.26)',color:'#8B5E3C',fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase',cursor:'pointer',textDecoration:'none',transition:'all .2s'}}
          onMouseEnter={e=>{const b=e.currentTarget as HTMLAnchorElement;b.style.background='rgba(122,170,192,.2)';b.style.borderColor='rgba(122,170,192,.42)'}}
          onMouseLeave={e=>{const b=e.currentTarget as HTMLAnchorElement;b.style.background='rgba(122,170,192,.12)';b.style.borderColor='rgba(122,170,192,.26)'}}>
          <BkArrow/> Înapoi
        </a>
      </header>

      {phase!=='invite' && <IntroScreen onStart={startBounce} phase={phase}/>}
