...
//       WHERE id = ${oid} AND email = ${session.email}
//       LIMIT 1
//     `;
...
//         wedding_date, wedding_time, location_name, google_maps_url, waze_url,religious_maps_url,
...
//         gallery_status, photos_expires_at, photo_consent_accepted, photos_activated_at
//       )
//       VALUES (
...
//         ${clean(body.wazeUrl)}, ${clean(body.religiousMaps)}, ${clean(body.religiousDate)}, ${clean(body.religiousTime)},
...
//         ${body.gallery_status || null},
//         ${clean(body.photos_expires_at)},
//         ${body.photo_consent_accepted ?? false},
//         ${clean(body.photos_activated_at)}
//       )
...
//         religious_maps_url = COALESCE(EXCLUDED.religious_maps_url, wedding_settings.religious_maps_url),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await sql`
      ...
      VALUES (
        ${oid}, ${clean(body.customSlug)}, ${clean(body.brideName)}, ${clean(body.groomName)},
        ${clean(body.nasiNames)}, ${clean(body.parentsNames)},
        ${clean(body.parents_bride)}, ${clean(body.parents_groom)},
        ${clean(body.weddingDate)},
        ${clean(body.weddingTime)}, ${clean(body.locationName)}, ${clean(body.googleMapsUrl)},
        ${clean(body.wazeUrl)}, ${clean(body.religiousMaps)}, ${clean(body.religiousDate)}, ${clean(body.religiousTime)},
        ${clean(body.religiousLocation)}, ${clean(body.religiousWaze)},
        ${body.isReligiousActive ?? null}, ${body.isMenuActive ?? null},
      ...
      ON CONFLICT (order_id) DO UPDATE SET
        ...
        parents_bride = EXCLUDED.parents_bride,
        parents_groom = EXCLUDED.parents_groom,
        wedding_date = COALESCE(EXCLUDED.wedding_date, wedding_settings.wedding_date),
        wedding_time = COALESCE(EXCLUDED.wedding_time, wedding_settings.wedding_time),
        location_name = COALESCE(EXCLUDED.location_name, wedding_settings.location_name),
        google_maps_url = COALESCE(EXCLUDED.google_maps_url, wedding_settings.google_maps_url),
