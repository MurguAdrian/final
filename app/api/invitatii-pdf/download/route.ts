...
<body>
<div class="inv">
  <svg class="corner tl" viewBox="0 0 90 90" fill="none">
    <path d="M4 86 L4 4 L86 4" stroke="#C9A84C" stroke-width="1.5" fill="none"/>
    <path d="M4 86 Q12 64 4 46 Q12 28 22 14 Q34 4 46 4" stroke="#C9A84C" stroke-width=".8" fill="none" opacity=".5"/>
  ...
  <svg class="corner tr" viewBox="0 0 90 90" fill="none">
    <path d="M4 86 L4 4 L86 4" stroke="#C9A84C" stroke-width="1.5" fill="none"/>
    <path d="M4 86 Q12 64 4 46 Q12 28 22 14 Q34 4 46 4" stroke="#C9A84C" stroke-width=".8" fill="none" opacity=".5"/>
    <circle cx="4" cy="4" r="4" fill="#C9A84C" opacity=".7"/>
  </svg>
  <svg class="corner bl" viewBox="0 0 90 90" fill="none">
    <path d="M4 86 L4 4 L86 4" stroke="#C9A84C" stroke-width="1.5" fill="none"/>
    <path d="M4 86 Q12 64 4 46 Q12 28 22 14 Q34 4 46 4" stroke="#C9A84C" stroke-width=".8" fill="none" opacity=".5"/>
    <circle cx="4" cy="4" r="4" fill="#C9A84C" opacity=".7"/>
  </svg>
  <svg class="corner br" viewBox="0 0 90 90" fill="none">
    <path d="M4 86 L4 4 L86 4" stroke="#C9A84C" stroke-width="1.5" fill="none"/>
    <path d="M4 86 Q12 64 4 46 Q12 28 22 14 Q34 4 46 4" stroke="#C9A84C" stroke-width=".8" fill="none" opacity=".5"/>
    <circle cx="4" cy="4" r="4" fill="#C9A84C" opacity=".7"/>
  </svg>
...
async function generateWithPuppeteer(html: string, format: string) {
  ...
  if (format === 'jpg') {
    ...
    await browser.close()
    return { data: Buffer.from(shot), contentType: 'image/jpeg' }
  }
  ...
  await browser.close()
  return { data: Buffer.from(pdf), contentType: 'application/pdf' }
}
...
export async function GET(req: Request) {
  ...
  if (!sessionId) {
    return new NextResponse('Session ID lipsă', { status: 400 })
  }
  ...
  try {
    ...
    if (session.payment_status !== 'paid') {
      return new NextResponse('Plata nu a fost confirmată', { status: 402 })
    }

    if (session.metadata?.paymentType !== 'invitatie_pdf') {
      return new NextResponse('Tip plată invalid', { status: 400 })
    }

    const fields = JSON.parse(session.metadata?.fields || '{}')
    const html = buildHTML(fields)
    const { data, contentType } = await generateWithPuppeteer(html, format)
    ...
    return new NextResponse('Eroare la generare', { status: 500 })
  }
}