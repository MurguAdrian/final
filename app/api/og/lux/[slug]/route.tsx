    ...
    return new ImageResponse(
      (
        <div style={{
          ...
          display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center',
          background: '#0A0803', color: '#D4AF37',
          fontSize: 28, gap: 20,
        }}>
          <div style={{ display: 'flex' }}>slug: {params.slug}</div>
          <div style={{ display: 'flex' }}>origin: {baseUrl}</div>
          <div style={{ display: 'flex' }}>font ok: {String(fontOk)}</div>
          <div style={{ display: 'flex' }}>font error: {fontError}</div>
        </div>
      ),
      { width: 1200, height: 630 }
    );
  } catch (e) {
    return new Response(`CATCH: ${String(e)}`, { status: 500 });
  }
}