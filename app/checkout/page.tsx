...
export default function CheckoutPage() {
  ...
  const handlePayment = async (priceId: string, themeName: string) => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        // AICI prindem eroarea de "Email existent" de la backend
  ...
  return (
    <div style={{ padding: "60px 20px", textAlign: "center", fontFamily: "sans-serif" }}>
      <h1>Alege tema preferată</h1>
      <p style={{ color: "#666", marginBottom: "30px" }}>Introdu email-ul pentru a începe.</p>
      ...
      <input 
        ...
        style={{ 
          ...
          marginBottom: "20px", borderRadius: "8px", border: "2px solid #eee", color: "black"
        }}
      />

      <div style={{ marginBottom: '40px', maxWidth: '400px', margin: '0 auto 40px', textAlign: 'left' }}>
        <label style={{ display: 'flex', gap: '10px', cursor: 'pointer' }}>
          <input type="checkbox" checked={accepted} onChange={(e) => setAccepted(e.target.checked)} />
          <span style={{ fontSize: '0.8rem', color: '#555' }}>Sunt de acord cu Termenii și Condițiile.</span>
        </label>
      </div>
      ...
      <div style={{ display: "flex", gap: "20px", justifyContent: "center" }}>
        ...
        <button 
          onClick={() => handlePayment("price_1TQR40DLRG6cKGjIoripBApH", "Lux")}
          disabled={loading !== ""}
          style={{ padding: "15px 30px", background: "black", color: "white", borderRadius: "8px", cursor: "pointer", opacity: accepted ? 1 : 0.5 }}
        >
          {loading === "Lux" ? "Se verifică..." : "Alege LUX"}
        </button>
      </div>
    </div>
  );
