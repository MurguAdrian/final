...
const GCSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ${KEYFRAMES}
  ::selection { background: rgba(228,107,174,.22); color: ${C.text}; }
  .ast-input { transition: border-color .2s, box-shadow .2s; }
  .ast-input:focus { border-color: rgba(228,107,174,.6) !important; box-shadow: ${SH.glow}; outline: none; }
  .ast-ghost:hover { border-color: rgba(191,167,242,.4) !important; color: ${C.text} !important; }

  .ast-tab { transition: color .18s, border-color .18s, background .18s; }
  .ast-tab:hover:not(.ast-tab-active) {
    color: ${C.accentDark} !important;
    background: rgba(228,107,174,.06) !important;
  }
  .ast-tab-active {
    color: ${C.accentDark} !important;
  ...
  .ast-header {
    ...
    border-bottom: 1px solid rgba(228,107,174,.16);
  ...
  .ast-invite-banner {
    ...
    border-bottom: 1px solid rgba(228,107,174,.14);
  ...
  .ast-tab-bar {
    ...
    border-bottom: 1px solid rgba(191,167,242,.16);
  }
  ...
  .ast-copy-btn {
    ...
    box-shadow: 0 6px 22px rgba(196,74,140,.3);
    transition: transform .2s, box-shadow .2s; flex-shrink: 0;
  }
  .ast-copy-btn:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(196,74,140,.45); }

  .ast-open-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(228,107,174,.08); color: ${C.accentDark};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
    border: 1px solid rgba(228,107,174,.28);
    cursor: pointer; text-decoration: none; flex-shrink: 0;
    transition: background .2s, border-color .2s;
  }
  .ast-open-btn:hover { background: rgba(228,107,174,.16); border-color: rgba(228,107,174,.45); }

  .ast-share-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(191,167,242,.08); color: ${C.textMuted};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
    border: 1px solid rgba(191,167,242,.22);
    cursor: pointer; flex-shrink: 0;
    transition: background .2s, border-color .2s, color .2s;
  }
  .ast-share-btn:hover { background: rgba(228,107,174,.14); border-color: rgba(228,107,174,.38); color: ${C.accentDark}; }
          ...
          <div className="ast-invite-banner">
            <div className="ast-invite-inner">
              <div style={{
                ...
                background: 'rgba(228,107,174,.08)',
                border: '1px solid rgba(228,107,174,.2)',
                borderRadius: BR.pill,
                flexShrink: 0,
              }}>
