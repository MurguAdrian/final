...
export default function MenuSection({ menuDetails }: { menuDetails: any }) {
  const [open, setOpen] = useState(false)

  // Logică identică cu originalul: filtrăm categoriile active care au cel puțin un item
  const activeCategories = menuDetails?.categories?.filter(
    (cat: any) => cat.active && cat.items && cat.items.length > 0
  ) ?? []

  // Dacă nu există categorii active cu items, nu afișăm nimic
    ...
    <>
      <style>{`
        ...
        .menu-trigger-btn {
          ...
          box-shadow: 0 8px 28px rgba(212,175,55,.32);
          position: relative;
          overflow: hidden;
        ...
        .menu-trigger-btn::after {
          ...
          animation: shimmerMenu 3s linear infinite;
        }
        .menu-trigger-btn:hover {
          transform: translateY(-2px);
          box-shadow: 0 14px 44px rgba(212,175,55,.55);
        }
        .menu-close-btn {
          background: none;
        ...
        .menu-item-card:hover { border-color: rgba(212,175,55,.22); }
        .menu-modal-scroll::-webkit-scrollbar { width: 4px; }
        .menu-modal-scroll::-webkit-scrollbar-track { background: transparent; }
        .menu-modal-scroll::-webkit-scrollbar-thumb { background: rgba(212,175,55,.25); border-radius: 2px; }
      `}</style>
      ...
      <div style={{
        ...
        {(['topleft','topright','bottomleft','bottomright'] as const).map(pos => (
          <div key={pos} style={{
            ...
            width: 13, height: 13,
            borderTop:    pos.includes('top')    ? '1px solid rgba(212,175,55,.35)' : 'none',
            borderBottom: pos.includes('bottom') ? '1px solid rgba(212,175,55,.35)' : 'none',
            borderLeft:   pos.includes('left')   ? '1px solid rgba(212,175,55,.35)' : 'none',
            borderRight:  pos.includes('right')  ? '1px solid rgba(212,175,55,.35)' : 'none',
            pointerEvents: 'none',
          }}/>
        ))}
        ...
        <div style={{
          ...
          <svg viewBox="0 0 48 48" fill="none" style={{ width: 34, height: 34 }}>
            <path d="M8 12 L8 40 Q8 44 12 44 L36 44 Q40 44 40 40 L40 12 Q40 8 36 8 L12 8 Q8 8 8 12Z"
              stroke="#D4AF37" strokeWidth="1.6" strokeOpacity=".65" fill="none"/>
            <path d="M16 18 L32 18 M16 24 L32 24 M16 30 L26 30"
              stroke="#D4AF37" strokeWidth="1.4" strokeOpacity=".55" strokeLinecap="round"/>
          </svg>
        </div>
        ...
        <h3 style={{
          ...
          color: '#F5E6A8', marginBottom: 10, lineHeight: 1.2,
        }}>
        ...
        <p style={{
          ...
          Descoperă preparatele alese cu grijă pentru această seară specială
        </p>

        <button className="menu-trigger-btn" onClick={() => setOpen(true)}>
          <span style={{ position: 'relative', zIndex: 1 }}>◆ Vezi Meniul</span>
        </button>
      </div>
      ...
      {open && (
        ...
        >
          <div
            ...
            className="menu-modal-scroll"
            style={{
              background: 'linear-gradient(170deg,#1A1408 0%,#0D0A04 100%)',
              borderRadius: 24,
              border: '1px solid rgba(212,175,55,.22)',
              boxShadow: '0 40px 120px rgba(0,0,0,.95), 0 0 60px rgba(212,175,55,.08)',
              maxWidth: 520, width: '100%',
              maxHeight: '88vh', overflowY: 'auto',
              animation: 'menuReveal .35s cubic-bezier(.4,0,.2,1)',
              position: 'relative',
              scrollbarWidth: 'thin',
            }}
          >
            ...
            {/* ── Modal Header (sticky) ── */}
            <div style={{
              padding: 'clamp(28px,4vw,40px) clamp(24px,4vw,36px) 22px',
              textAlign: 'center',
              borderBottom: '1px solid rgba(212,175,55,.1)',
              position: 'sticky', top: 0,
              background: 'linear-gradient(170deg,#1A1408,#0D0A04)',
              zIndex: 10,
            }}>
              {/* Mini crown */}
              <svg viewBox="0 0 120 60" fill="none" style={{ width: 60, height: 30, display: 'block', margin: '0 auto 12px' }}>
                <path d="M10 50 L10 20 L30 40 L60 5 L90 40 L110 20 L110 50 Z"
                  fill="none" stroke="url(#mcg)" strokeWidth="1.4" strokeLinejoin="round"/>
                <circle cx="60" cy="5" r="3.5" fill="url(#mcg)"/>
                <circle cx="30" cy="40" r="2.5" fill="url(#mcg)"/>
                <circle cx="90" cy="40" r="2.5" fill="url(#mcg)"/>
                <path d="M4 50 L116 50" stroke="url(#mcg)" strokeWidth="1"/>
                <defs>
                  <linearGradient id="mcg" x1="0" y1="0" x2="120" y2="0" gradientUnits="userSpaceOnUse">
              ...
              <p style={{
                ...
                Selecție rafinată · {activeCategories.length} {activeCategories.length === 1 ? 'categorie' : 'categorii'}
              </p>
            </div>

            {/* ── Categories ── */}
            <div style={{ padding: 'clamp(24px,3vw,36px) clamp(20px,3vw,32px)' }}>
              {activeCategories.map((cat: any, cIdx: number) => (
                <div key={cIdx} style={{ marginBottom: cIdx < activeCategories.length - 1 ? 36 : 0 }}>
                  ...
                  <div style={{
                    ...
                    marginBottom: 16,
                    paddingBottom: 14,
                    borderBottom: '1px solid rgba(212,175,55,.13)',
                  }}>
                    <div style={{
                    ...
                    <h4 style={{
                      ...
                      color: '#D4AF37', fontWeight: 400, margin: 0,
                    }}>
                      {cat.label}
                    </h4>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {cat.items.map((item: any, iIdx: number) => (
                      <div key={iIdx} className="menu-item-card">
                        <div style={{
                          fontFamily: "'Cormorant Garamond', serif",
                  ...
                  {cIdx < activeCategories.length - 1 && (
                    <div style={{ display: 'flex', alignItems: 'center', marginTop: 28 }}>
                      <div style={{ flex: 1, height: 1, background: 'linear-gradient(90deg,transparent,rgba(212,175,55,.18))' }}/>
                      <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'rgba(212,175,55,.35)', margin: '0 12px' }}/>
                      <div style={{ flex: 1, height: 1, background: 'linear-gradient(90deg,rgba(212,175,55,.18),transparent)' }}/>
                    </div>
                  )}
                </div>
              ))}
