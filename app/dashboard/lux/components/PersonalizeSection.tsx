  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (data?.weddingDetails) {
          const d = data.weddingDetails;
          // MAPARE STRICTĂ: Baza de date (snake_case) -> Formular (camelCase)
          setFormData({
            customSlug: d.custom_slug || '',
            brideName: d.bride_name || '',
            groomName: d.groom_name || '',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { 
        ...
        alert(err.error === "SLUG_TAKEN" ? "Acest URL este deja luat!" : "Eroare la salvare.");
      }
  ...
  return (
    <div style={{ maxWidth: '900px', paddingBottom: '100px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px' }}>
        ...
        <a href={`/invitatie/lux/${formData.customSlug}`} target="_blank" style={previewBtn}>👁️ PREVIZUALIZARE LIVE</a>
      </div>

      <form onSubmit={handleSave}>
        <div style={sectionBox}>
           <label style={labS}>URL PERSONALIZAT (Slug)</label>
        ...
        <div style={grid2}>
          ...
          <div style={sectionBox}>
            <h3 style={secTitle}>Petrecere</h3>
        ...
        <div style={{...sectionBox, marginTop: '20px'}}>
          ...
          <div style={grid2}>
            ...
            <input placeholder="Locație Biserică" style={inpS} value={formData.religiousLocation} onChange={e => setFormData({...formData, religiousLocation: e.target.value})} />
        ...
        <div style={grid2}>
          ...
          <div style={sectionBox}>
            ...
            <label style={checkL}><input type="checkbox" checked={formData.isAccommodationActive} onChange={e => setFormData({...formData, isAccommodationActive: e.target.checked})} /> Oferim Cazare</label>
            <label style={checkL}><input type="checkbox" checked={formData.isTransportActive} onChange={e => setFormData({...formData, isTransportActive: e.target.checked})} /> Oferim Transport</label>
          </div>
        </div>
        ...
        <button type="submit" disabled={loading} style={saveBtn}>{loading ? "SE SALVEAZĂ..." : "SALVEAZĂ TOATE DATELE"}</button>
      </form>
    </div>
  );
};

// CSS Intern pentru coerență
...
const checkL = { display: 'block', margin: '10px 0', cursor: 'pointer', fontSize: '0.9rem' };
const previewBtn = { padding: '10px 20px', border: '1px solid #d4af37', color: '#d4af37', textDecoration: 'none', borderRadius: '4px', fontWeight: 'bold' as any };
