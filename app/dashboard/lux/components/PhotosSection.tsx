...
// FIȘIER: andre/app/dashboard/lux/components/PhotosSection.tsx
...
const GoldDivider = () => (
  <div style={{ display: 'flex', alignItems: 'center', width: '100%', margin: `14px 0 ${SP.xl}px` }}>
    <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,transparent,rgba(212,175,55,.28))` }} />
    <svg viewBox="0 0 60 20" width="50" height="16" fill="none" style={{ flexShrink: 0 }}>
      ...
      <rect x="25" y="5" width="10" height="10" transform="rotate(45 30 10)" fill="none" stroke={C.rose} strokeWidth="1" strokeOpacity=".75" />
      <circle cx="30" cy="10" r="1.8" fill={C.rose} fillOpacity=".6" />
    </svg>
    <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,rgba(212,175,55,.28),transparent)` }} />
  </div>
);
...
const ph: Record<string, React.CSSProperties> = {
  ...
  headerLabel:       { fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.3em', textTransform: 'uppercase', color: 'rgba(212,175,55,.5)', marginBottom: SP.xs },
  ...
  card:              { background: 'rgba(212,175,55,.03)', border: `1px solid ${C.borderMed}`, borderRadius: BR.xl, overflow: 'hidden', marginBottom: 14 },
  ...
  consentLabel:      { display: 'flex', alignItems: 'flex-start', gap: BR.lg, cursor: 'pointer', padding: `12px 14px`, background: 'rgba(212,175,55,.04)', border: `1px solid ${C.borderMed}`, borderRadius: BR.md },
  checkbox:          { width: 18, height: 18, border: `1.5px solid rgba(212,175,55,.35)`, background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, transition: 'all .2s' },
  checkboxChecked:   { background: C.rose, borderColor: C.rose },
  consentText:       { fontFamily: F.heading, fontSize: FS.xs, letterSpacing: '.06em', color: 'rgba(212,175,55,.65)', lineHeight: 1.6 },
  roseBtn:           { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: SP.sm, padding: `12px ${SP.xl}px`, borderRadius: BR.sm, background: GR.roseBtn, color: '#0A0803', fontFamily: F.heading, fontSize: FS.base, fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', boxShadow: SH.btnRose, width: '100%' } as React.CSSProperties,
  ...
  gallerySubCount:   { fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.22em', textTransform: 'uppercase', color: 'rgba(212,175,55,.5)', marginTop: 2, marginBottom: 0 },
  ...
  photoImg:          { width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform .4s ease' } as React.CSSProperties,
  photoOverlay:      { position: 'absolute', inset: 0, background: `linear-gradient(to top,rgba(5,4,1,.88) 0%,rgba(5,4,1,.35) 50%,transparent 100%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 5, padding: SP.sm, opacity: 0, transition: 'opacity .3s ease' } as React.CSSProperties,
  dlBtn:             { display: 'inline-flex', alignItems: 'center', gap: 5, padding: `6px 12px`, borderRadius: BR.sm, background: 'rgba(212,175,55,.15)', border: `1px solid rgba(212,175,55,.28)`, color: C.rose, fontFamily: F.heading, fontSize: FS.tiny, fontWeight: 600, letterSpacing: '.1em', cursor: 'pointer', width: '100%', justifyContent: 'center', transition: 'all .2s' } as React.CSSProperties,
  viewBtn:           { display: 'inline-flex', alignItems: 'center', gap: 5, padding: `5px 12px`, borderRadius: BR.sm, background: 'rgba(212,175,55,.08)', border: `1px solid rgba(212,175,55,.2)`, color: 'rgba(212,175,55,.7)', fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.1em', textDecoration: 'none', width: '100%', justifyContent: 'center', transition: 'all .2s' } as React.CSSProperties,
  emptyState:        { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: `40px ${SP.xl}px`, background: 'rgba(212,175,55,.03)', border: '1px dashed rgba(212,175,55,.18)', borderRadius: BR.xl, textAlign: 'center' } as React.CSSProperties,
};
    ...
    <div style={ph.wrapper}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ghost-btn:hover                 { background: rgba(212,175,55,.1) !important; border-color: rgba(212,175,55,.45) !important; }
        .dl-btn:hover                    { background: rgba(212,175,55,.22) !important; }
          ...
          <div style={{ marginTop: SP.xxxl, animation: 'rm-fade-in .5s ease both' }}>
              ...
              <div className="gallery-header-btns" style={{ display: 'flex', gap: SP.sm, flexWrap: 'wrap' }}>
                {photos.length > 0 && (
                  <button type="button" onClick={handleDownloadAll} disabled={downloadingAll} className="rose-btn dl-all-btn" style={{ ...ph.roseBtn, padding: `9px 18px`, fontSize: FS.base, gap: 7, transition: 'all .25s', opacity: downloadingAll ? 0.7 : 1, width: 'auto' }}>
                    {downloadingAll
                      ? <><svg viewBox="0 0 16 16" fill="none" style={{ width: IS.sm, height: IS.sm, animation: 'rm-spin 1s linear infinite', flexShrink: 0 }}><circle cx="8" cy="8" r="6" stroke="#0A0803" strokeWidth="1.8" strokeDasharray="20 10" /></svg>Descărcând...</>
                      : <><svg viewBox="0 0 16 16" fill="none" style={{ width: IS.sm, height: IS.sm, flexShrink: 0 }}><path d="M8 2 L8 11 M4 8 L8 12 L12 8" stroke="#0A0803" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /><path d="M2 14 L14 14" stroke="#0A0803" strokeWidth="1.8" strokeLinecap="round" /></svg>Descarcă Toate pozele</>
                    }
                  </button>
                )}
            ...
            {photos.length === 0 && !loading ? (
              <div style={ph.emptyState}>
                ...
                <p style={{ fontFamily: F.body, fontSize: FS.base + 2, fontStyle: 'italic', color: 'rgba(212,175,55,.38)', marginBottom: SP.xs }}>Nicio fotografie încărcată de invitați</p>
                <p style={{ fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.18em', color: 'rgba(212,175,55,.22)', marginBottom: 0 }}>IMAGINILE TRIMISE DE INVITAȚI VOR APĂREA INSTANT AICI</p>
              </div>
            ) : (
              <div className="gallery-grid">
                {photos.map((p: Photo) => (
                    ...
                    <div className="photo-overlay" style={ph.photoOverlay}>
                      ...
                      <a href={p.url} target="_blank" rel="noreferrer" style={ph.viewBtn}>
                        <svg viewBox="0 0 16 16" fill="none" style={{ width: IS.sm, height: IS.sm, flexShrink: 0 }}>
                          <path d="M1 8C1 8 3.5 3 8 3s7 5 7 5-2.5 5-7 5S1 8 1 8z" stroke="rgba(212,175,55,.7)" strokeWidth="1.4" />
                          <circle cx="8" cy="8" r="2" stroke="rgba(212,175,55,.7)" strokeWidth="1.4" />
                        </svg>
                        Vezi Full
                      </a>
                    </div>
