...
export const PhotosSection = ({ initialData, orderId, onSave }: any) => {
  ...
  const handleActivate = async () => {
    if (initialData?.photos_expires_at) {
      alert("Galeria a fost deja activată anterior.");
      return;
    }
    const confirm = window.confirm("Activezi galeria pentru 3 zile gratuit?");
    if (!confirm) return;
    ...
    try {
      await fetch('/api/dashboard/personalize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ orderId, isPhotosActive: true, gallery_status: 'active', photos_expires_at: expiresAt }),
      });
      onSave(); 
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease' }}>
      ...
      {status === 'inactive' && (
        <div style={cardS}>
          <h3>Modul Inactiv</h3>
          <p style={{ color: '#888', marginBottom: '20px' }}>Activează galeria doar în ziua nunții (3 zile gratis).</p>
          <button onClick={handleActivate} style={btnGoldFull}>ACTIVEAZĂ GALERIA</button>
        </div>
      )}
      ...
      {status === 'active' && (
        <div style={cardS}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ color: '#4caf50', margin: 0 }}>🟢 Modul Activ</h3>
            <span style={{ color: '#ffa500', fontWeight: 'bold' }}>⏳ {timeLeft}</span>
          </div>
          <button onClick={() => handlePayment('extend')} style={{ ...btnGoldFull, marginTop: '20px' }}>PRELUNGEȘTE CU 5 ZILE (50 RON)</button>
        </div>
      )}

      {(status === 'expired' || status === 'archived') && status !== 'deleted' && !initialData?.is_unlock_paid && (
        <div style={{...cardS, border: '1px solid #ff4444'}}>
          <h3 style={{ color: '#ff4444', margin: 0 }}>🔴 Timpul a expirat</h3>
          <p style={{ color: '#888', marginTop: '10px' }}>
            Pozele sunt în Arhivă până la: <strong>{initialData.archive_expires_at ? new Date(initialData.archive_expires_at).toLocaleDateString('ro-RO') : 'N/A'}</strong>.
          </p>
          <button onClick={() => handlePayment('unlock')} style={{ ...btnGoldFull, background: '#ff4444', color: '#fff', border: 'none', marginTop: '20px' }}>DEBLOCHEAZĂ ACCESUL (100 RON)</button>
        </div>
      )}

      {status === 'deleted' && (
        <div style={{...cardS, border: '2px solid #555', background: '#050505'}}>
          <h3 style={{ color: '#888', margin: 0 }}>🔒 Galerie Dezactivată Permanent</h3>
          <p style={{ color: '#666', marginTop: '15px', lineHeight: '1.6' }}>
            Perioada de 30 de zile a expirat. Pozele au fost șterse definitiv conform politicilor noastre.
          </p>
        </div>
      )}
...
const cardS = { background: '#111', padding: '30px', borderRadius: '12px', border: '1px solid #d4af3733' };
const btnGoldFull = { width: '100%', padding: '15px', background: '#d4af37', color: '#000', fontWeight: 'bold' as any, border: 'none', borderRadius: '4px', cursor: 'pointer' };
const refreshBtn = { background: 'transparent', border: '1px solid #d4af37', color: '#d4af37', padding: '8px 20px', borderRadius: '4px', cursor: 'pointer' };
const galleryGrid = { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' };
const photoWrapper = { position: 'relative' as any, height: '200px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #222' };
const imgS = { width: '100%', height: '100%', objectFit: 'cover' as any };
const downloadOverlay = { position: 'absolute' as any, bottom: 0, width: '100%', background: 'rgba(212, 175, 55, 0.9)', color: '#000', textAlign: 'center' as any, padding: '8px', fontSize: '0.7rem' };