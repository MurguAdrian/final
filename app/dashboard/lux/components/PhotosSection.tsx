...
// export const PhotosSection = ({ initialData, orderId, onSave }: PhotosSectionProps) => {
//   const [status, setStatus]               = useState<GalleryStatus>(initialData?.gallery_status || 'inactive');
...
//     if (initialData?.photos_expires_at && initialData?.gallery_status === 'active') {
//       const timer = setInterval(() => {
//         const diff = +new Date(initialData.photos_expires_at) - Date.now();
//         if (diff <= 0) {
//           setTimeLeft('EXPIRAT');
//           setStatus('expired');
//           clearInterval(timer);
...
// const showGallery =
//   (status === 'active' || status === 'expired' || initialData?.is_unlock_paid === true)
//   && status !== 'deleted';
//   const isExtendable = initialData?.photos_expires_at && initialData?.photos_activated_at
//     && new Date(initialData.photos_expires_at).getTime() - new Date(initialData.photos_activated_at).getTime()
//        < 4 * 24 * 60 * 60 * 1000;
...
export const PhotosSection = ({ initialData: _initialData, orderId, onSave }: PhotosSectionProps) => {
  const [initialData, setInitialData]       = useState(_initialData);
  const [status, setStatus]                 = useState<GalleryStatus>(_initialData?.gallery_status || 'inactive');
  const [photos, setPhotos]                 = useState<Photo[]>([]);
  const [loading, setLoading]               = useState(false);
  const [timeLeft, setTimeLeft]             = useState('');
  const [consentChecked, setConsentChecked] = useState(false);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // 1. HARD LIMIT – prioritate maximă
      if (now >= hardLimit) {
        setStatus('deleted');
        setTimeLeft('ȘTERS DEFINITIV');
        return;
  ...
  // ─────────────────────────────
  // ACTIVATE – consent + personalize (fără Stripe, ca înainte)
  // ─────────────────────────────
  const handleActivate = async () => {
    if (!consentChecked) { alert('Trebuie să accepți termenii pentru a activa.'); return; }
    try {
      ...
      await fetchSettings(); // reîncarcă din DB, nu reload
    } catch (e) { console.error(e); }
  };
  ...
  // Extend disponibil DOAR în primele 3 zile de la T0
  const isExtendable = (() => {
    if (!initialData?.photos_activated_at || initialData?.is_unlock_paid) return false;
    const T0 = new Date(initialData.photos_activated_at).getTime();
    return Date.now() < T0 + 3 * 86400000;
  })();
...
const styles: Record<string, React.CSSProperties> = {
  ...
  statusBadgeExpired: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 14px', borderRadius: 100, background: 'rgba(220,60,60,.1)', border: '1px solid rgba(220,60,60,.25)', fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: '.18em', textTransform: 'uppercase', color: '#ef9a9a' },
  statusBadgeDeleted: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 14px', borderRadius: 100, background: 'rgba(212,175,55,.06)', border: `1px solid ${G.goldBorder}`, fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: '.18em', textTransform: 'uppercase', color: G.goldDim },
