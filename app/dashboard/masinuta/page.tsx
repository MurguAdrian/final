...
import { C, F, FS, SP, BR, LY, GR, SH, KEYFRAMES, FONTS_IMPORT } from './masinutaTokens';
...
const GCSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ${KEYFRAMES}
  ::selection { background: rgba(74,139,194,.25); color: ${C.text}; }
  .ast-input { transition: border-color .2s, box-shadow .2s; }
  .ast-input:focus { border-color: rgba(74,139,194,.6) !important; box-shadow: ${SH.glow}; outline: none; }
  .ast-ghost:hover { border-color: rgba(74,139,194,.4) !important; color: ${C.text} !important; }

  .ast-tab { transition: color .18s, border-color .18s, background .18s; }
  .ast-tab:hover:not(.ast-tab-active) {
    color: ${C.accentDark} !important;
    background: rgba(74,139,194,.06) !important;
  }
  .ast-tab-active {
    color: ${C.accentDark} !important;
    border-bottom-color: ${C.accentDark} !important;
    background: rgba(74,139,194,.06) !important;
  }
  ...
  .ast-header {
    ...
    border-bottom: 1px solid rgba(74,139,194,.16);
  ...
  .ast-invite-banner {
    ...
    border-bottom: 1px solid rgba(74,139,194,.14);
  ...
  .ast-copy-btn {
    ...
    box-shadow: 0 6px 22px rgba(46,98,153,.3);
    transition: transform .2s, box-shadow .2s; flex-shrink: 0;
  }
  .ast-copy-btn:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(46,98,153,.45); }

  .ast-open-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(74,139,194,.08); color: ${C.accentDark};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
    border: 1px solid rgba(74,139,194,.28);
    cursor: pointer; text-decoration: none; flex-shrink: 0;
    transition: background .2s, border-color .2s;
  }
  .ast-open-btn:hover { background: rgba(74,139,194,.16); border-color: rgba(74,139,194,.45); }

  .ast-share-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(74,139,194,.06); color: ${C.textMuted};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
    border: 1px solid rgba(74,139,194,.2);
    cursor: pointer; flex-shrink: 0;
    transition: background .2s, border-color .2s, color .2s;
  }
  .ast-share-btn:hover { background: rgba(74,139,194,.14); border-color: rgba(74,139,194,.38); color: ${C.accentDark}; }
...
export default function MasinutaDashboard() {
  ...
  const inviteLink     = customSlug ? `https://www.vibeinvite.ro/invitatie/masinuta/${customSlug}` : '';
        ...
        <header className="ast-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: SP.sm, minWidth: 0 }}>
            ...
            <span style={{
              ...
              <span style={{ color: C.textMuted, fontWeight: 400, fontSize: FS.tiny, marginLeft: SP.sm }}>mașinuță</span>
            </span>
          </div>
          ...
          <div className="ast-invite-banner">
            <div className="ast-invite-inner">
              <div style={{
                ...
                background: 'rgba(74,139,194,.08)',
                border: '1px solid rgba(74,139,194,.18)',
                borderRadius: BR.pill,
                flexShrink: 0,
              }}>
              ...
              <span className="ast-invite-link" style={{
                fontFamily: F.mono, fontSize: FS.sm, color: C.accentDark,
              ...
              <button onClick={copy} className="ast-copy-btn">
                {copied
                  ...
                  : <><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ width: 11, height: 11 }}><rect x="4" y="4" width="9" height="9" rx="1.5" /><path d="M3 10V3a1 1 0 0 1 1-1h7" /></svg> Copiază</>
                }
              </button>
            ...
            {([
              ...
              <button
                ...
                style={{
                  ...
                  borderBottom: `2.5px solid ${tab === key ? C.accentDark : 'transparent'}`,
                  display: 'flex', alignItems: 'center', gap: SP.xs + 2,
