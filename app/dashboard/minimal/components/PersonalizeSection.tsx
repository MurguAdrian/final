  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire({
          title: '<span style="color: #1a1a1a; font-family: sans-serif;">Salvat</span>',
        ...
        if (res.status === 409 || err.error?.toLowerCase().includes('exist') || err.error?.toLowerCase().includes('link')) {
          Swal.fire({
            title: '<span style="color: #1a1a1a; font-family: sans-serif;">Link rezervat</span>',
...
.ps-gen-btn:hover:not(:disabled) {
  background: rgba(0, 0, 0, .05) !important;       /* Un gri extrem de fin pe fundal */
  border-color: rgba(0, 0, 0, .3) !important;      /* Chenar discret */
  color: rgba(30, 30, 30, .95) !important;         /* Text aproape negru, foarte lizibil */
}
.ps-test-btn:hover {
  background: rgba(0, 0, 0, .06) !important;
  border-color: rgba(0, 0, 0, .25) !important;
  color: rgba(30, 30, 30, .95) !important;
}
