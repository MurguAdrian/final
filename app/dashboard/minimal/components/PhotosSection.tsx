// FIȘIER: andre/app/dashboard/minimal/components/PhotosSection.tsx
...
const MinimalDivider = () => (
  <div style={{ display: 'flex', alignItems: 'center', width: '100%', margin: `14px 0 ${SP.xl}px` }}>
    <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,transparent,rgba(26,26,26,.15))` }} />
    ...
    <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,rgba(26,26,26,.15),transparent)` }} />
  </div>
);
...
const ph: Record<string, React.CSSProperties> = {
  ...
  headerLabel:       { fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.3em', textTransform: 'uppercase', color: 'rgba(26,26,26,.4)', marginBottom: SP.xs },
  ...
  card:              { background: 'rgba(26,26,26,.02)', border: `1px solid ${C.borderMed}`, borderRadius: BR.xl, overflow: 'hidden', marginBottom: 14 },
  ...
  consentLabel:      { display: 'flex', alignItems: 'flex-start', gap: BR.lg, cursor: 'pointer', padding: `12px 14px`, background: 'rgba(26,26,26,.03)', border: `1px solid ${C.borderMed}`, borderRadius: BR.md },
  checkbox:          { width: 18, height: 18, border: `1.5px solid rgba(26,26,26,.25)`, background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, transition: 'all .2s' },
  checkboxChecked:   { background: C.rose, borderColor: C.rose },
  consentText:       { fontFamily: F.heading, fontSize: FS.xs, letterSpacing: '.06em', color: 'rgba(26,26,26,.55)', lineHeight: 1.6 },
  ...
  gallerySubCount:   { fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.22em', textTransform: 'uppercase', color: 'rgba(26,26,26,.4)', marginTop: 2, marginBottom: 0 },
  ...
  photoOverlay:      { position: 'absolute', inset: 0, background: `linear-gradient(to top,rgba(15,15,15,.88) 0%,rgba(15,15,15,.35) 50%,transparent 100%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 5, padding: SP.sm, opacity: 0, transition: 'opacity .3s ease' } as React.CSSProperties,
  dlBtn:             { display: 'inline-flex', alignItems: 'center', gap: 5, padding: `6px 12px`, borderRadius: BR.sm, background: 'rgba(255,255,255,.85)', border: `1px solid rgba(26,26,26,.2)`, color: C.rose, fontFamily: F.heading, fontSize: FS.tiny, fontWeight: 600, letterSpacing: '.1em', cursor: 'pointer', width: '100%', justifyContent: 'center', transition: 'all .2s' } as React.CSSProperties,
  viewBtn:           { display: 'inline-flex', alignItems: 'center', gap: 5, padding: `5px 12px`, borderRadius: BR.sm, background: 'rgba(255,255,255,.55)', border: `1px solid rgba(26,26,26,.15)`, color: 'rgba(26,26,26,.65)', fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.1em', textDecoration: 'none', width: '100%', justifyContent: 'center', transition: 'all .2s' } as React.CSSProperties,
  emptyState:        { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: `40px ${SP.xl}px`, background: 'rgba(26,26,26,.02)', border: '1px dashed rgba(26,26,26,.12)', borderRadius: BR.xl, textAlign: 'center' } as React.CSSProperties,
};
    ...
    <div style={ph.wrapper}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ghost-btn:hover                 { background: rgba(26,26,26,.08) !important; border-color: rgba(26,26,26,.3) !important; }
        .dl-btn:hover                    { background: rgba(26,26,26,.06) !important; }
          ...
          <div style={{ marginTop: SP.xxxl, animation: 'rm-fade-in .5s ease both' }}>
            ...
            {photos.length === 0 && !loading ? (
              <div style={ph.emptyState}>
                <svg viewBox="0 0 48 48" fill="none" style={{ width: 38, height: 38, marginBottom: BR.lg, opacity: .25 }}>
                ...
                <p style={{ fontFamily: F.body, fontSize: FS.base + 2, fontStyle: 'italic', color: 'rgba(26,26,26,.3)', marginBottom: SP.xs }}>Nicio fotografie încărcată de invitați</p>
                <p style={{ fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.18em', color: 'rgba(26,26,26,.18)', marginBottom: 0 }}>IMAGINILE TRIMISE DE INVITAȚI VOR APĂREA INSTANT AICI</p>
              </div>
            ) : (
                    ...
                    <div className="photo-overlay" style={ph.photoOverlay}>
                      ...
                      <a href={p.url} target="_blank" rel="noreferrer" style={ph.viewBtn}>
                        <svg viewBox="0 0 16 16" fill="none" style={{ width: IS.sm, height: IS.sm, flexShrink: 0 }}>
                          <path d="M1 8C1 8 3.5 3 8 3s7 5 7 5-2.5 5-7 5S1 8 1 8z" stroke="rgba(26,26,26,.65)" strokeWidth="1.4" />
                          <circle cx="8" cy="8" r="2" stroke="rgba(26,26,26,.65)" strokeWidth="1.4" />
                        </svg>
                        Vezi Full
                      </a>
                    </div>
