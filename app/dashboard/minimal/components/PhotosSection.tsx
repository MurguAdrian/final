    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: DARK, width: '100%', maxWidth: 900, margin: '0 auto', boxSizing: 'border-box', overflowX: 'hidden' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .mn-gold-btn:hover { background: #9a3e2d !important; transform: translateY(-1px); }
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, letterSpacing: '.3em', textTransform: 'uppercase', color: LIGHT, marginBottom: 0 }}>Panou Control</p>
            </div>
        ...
        {isFirstActivation && (
          <div style={{ background: '#fff', border: `1px solid ${RULE}`, borderTop: `3px solid ${ACCENT}`, marginBottom: 14, boxShadow: '0 2px 8px rgba(0,0,0,.04)' }}>
            <div style={{ padding: 'clamp(16px,3vw,24px)', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer', padding: '12px 14px', background: BG, border: `1px solid ${RULE}`, borderLeft: `3px solid rgba(200,80,58,.3)` }} onClick={() => setConsentChecked(!consentChecked)}>
                <div style={{ width: 18, height: 18, border: `1px solid ${consentChecked ? ACCENT : RULE}`, background: consentChecked ? ACCENT : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, transition: 'all .18s' }}>
                  {consentChecked && <svg viewBox="0 0 12 12" fill="none" style={{ width: 10, height: 10 }}><path d="M2 6L5 9L10 3" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                </div>
                <input type="checkbox" checked={consentChecked} onChange={() => {}} style={{ display: 'none' }} />
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: MID, lineHeight: 1.6 }}>Sunt de acord cu activarea modulului foto pe toată durata existenței contului (12 luni).</span>
              </label>
              <button type="button" className="mn-gold-btn" onClick={handleActivate} disabled={!consentChecked} style={{
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 20px',
                background: DARK, color: '#fff', border: 'none', cursor: consentChecked ? 'pointer' : 'not-allowed',
                fontFamily: "'DM Sans', sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: '.16em', textTransform: 'uppercase',
                opacity: consentChecked ? 1 : .4, transition: 'all .2s', width: '100%',
              }}>
          ...
          <div style={{ background: '#fff', border: `1px solid ${RULE}`, borderLeft: `3px solid #5a9a6a`, marginBottom: 14, boxShadow: '0 2px 8px rgba(0,0,0,.04)' }}>
            <div style={{ padding: 'clamp(14px,2.5vw,20px)', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '4px 12px', background: 'rgba(90,154,106,.08)', border: '1px solid rgba(90,154,106,.25)', fontFamily: "'DM Sans', sans-serif", fontSize: 9, letterSpacing: '.18em', textTransform: 'uppercase', color: '#5a9a6a' }}>
          ...
          <div style={{ marginTop: 24, animation: 'mn-fade-in .5s ease both' }}>
            <div className="gallery-header-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 12, flexWrap: 'wrap' }}>
              <div>
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, letterSpacing: '.22em', textTransform: 'uppercase', color: LIGHT, marginTop: 2, marginBottom: 0 }}>Albumul tău privat online</p>
              </div>
              <div className="gallery-header-btns" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {photos.length > 0 && (
                  <button type="button" onClick={handleDownloadAll} disabled={downloadingAll} className="mn-gold-btn dl-all-btn" style={{
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '9px 16px',
                    background: DARK, color: '#fff', border: 'none', cursor: downloadingAll ? 'not-allowed' : 'pointer',
                    fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase',
                    transition: 'all .2s', opacity: downloadingAll ? 0.7 : 1, width: 'auto',
                  }}>
                    {downloadingAll ? (
                      <><svg viewBox="0 0 16 16" fill="none" style={{ width: 12, height: 12, animation: 'mn-spin 1s linear infinite', flexShrink: 0 }}><circle cx="8" cy="8" r="6" stroke="#fff" strokeWidth="1.8" strokeDasharray="20 10" /></svg>Descărcând...</>
                    ) : (
                      <><svg viewBox="0 0 16 16" fill="none" style={{ width: 12, height: 12, flexShrink: 0 }}><path d="M8 2 L8 11 M4 8 L8 12 L12 8" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /><path d="M2 14 L14 14" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" /></svg>Descarcă Toate</>
                    )}
                  </button>
                )}
                <button type="button" onClick={fetchPhotos} className="mn-ghost-btn" style={{
                  display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 14px',
                  background: BG, border: `1px solid ${RULE}`, color: MID,
                  fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '.1em',
                  cursor: 'pointer', transition: 'all .2s', whiteSpace: 'nowrap',
                }}>
                  {loading ? (
                    <svg viewBox="0 0 16 16" fill="none" style={{ width: 12, height: 12, animation: 'mn-spin 1s linear infinite', flexShrink: 0 }}><circle cx="8" cy="8" r="6" stroke={ACCENT} strokeWidth="1.8" strokeDasharray="20 10" /></svg>
                  ) : (
                    <svg viewBox="0 0 16 16" fill="none" style={{ width: 12, height: 12, flexShrink: 0 }}><path d="M2 8C2 4.68 4.68 2 8 2s6 2.68 6 6-2.68 6-6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /><path d="M2 8 L4.5 5.5 M2 8 L4.5 10.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  )}
                  Actualizează
                </button>
              </div>
            ...
            {photos.length === 0 && !loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '36px 20px', background: '#fff', border: `1px dashed ${RULE}`, textAlign: 'center' }}>
                ...
                <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, fontStyle: 'italic', color: LIGHT, marginBottom: 4 }}>Nicio fotografie încărcată de invitați</p>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 8, letterSpacing: '.18em', textTransform: 'uppercase', color: '#CCCCCC', marginBottom: 0 }}>IMAGINILE TRIMISE DE INVITAȚI VOR APĂREA INSTANT AICI</p>
              </div>
            ) : (
              <div className="gallery-grid">
                {photos.map((p: Photo) => (
                  <div key={p.id} className="photo-card" style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden', border: `1px solid ${RULE}`, background: BG, cursor: 'pointer' }}>
                    <img src={p.url} alt="Amintire eveniment" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform .35s ease' }} />
                    <div className="photo-overlay" style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(17,17,17,.85) 0%,rgba(17,17,17,.3) 50%,transparent 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 5, padding: 8, opacity: 0, transition: 'opacity .25s ease' }}>
                      <button type="button" onClick={() => handleDownloadSingle(p)} disabled={downloadingId === p.id} className="mn-dl-btn" style={{
                        display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px',
                        background: 'rgba(247,244,240,.12)', border: `1px solid rgba(247,244,240,.3)`,
                        color: '#F7F4F0', fontFamily: "'DM Sans', sans-serif", fontSize: 9, fontWeight: 500, letterSpacing: '.08em',
                        cursor: 'pointer', width: '100%', justifyContent: 'center', transition: 'all .18s',
                      }}>
                        {downloadingId === p.id ? (
                          <svg viewBox="0 0 16 16" fill="none" style={{ width: 11, height: 11, animation: 'mn-spin 1s linear infinite', flexShrink: 0 }}><circle cx="8" cy="8" r="6" stroke="#F7F4F0" strokeWidth="1.8" strokeDasharray="20 10" /></svg>
                        ) : (
                          <svg viewBox="0 0 16 16" fill="none" style={{ width: 11, height: 11, flexShrink: 0 }}><path d="M8 2 L8 10 M4 7 L8 11 L12 7" stroke="#F7F4F0" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M2 13.5 L14 13.5" stroke="#F7F4F0" strokeWidth="1.5" strokeLinecap="round" /></svg>
                        )}
                      ...
                      <a href={p.url} target="_blank" rel="noreferrer" style={{
                        ...
                        color: 'rgba(247,244,240,.75)', fontFamily: "'DM Sans', sans-serif", fontSize: 9, letterSpacing: '.08em',
                        textDecoration: 'none', width: '100%', justifyContent: 'center', transition: 'all .18s',
                      }}>
                        <svg viewBox="0 0 16 16" fill="none" style={{ width: 11, height: 11, flexShrink: 0 }}><path d="M1 8C1 8 3.5 3 8 3s7 5 7 5-2.5 5-7 5S1 8 1 8z" stroke="currentColor" strokeWidth="1.3" /><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.3" /></svg>
                        Vezi Full
                      </a>
                    </div>
                  </div>
