...
interface CategoryCardProps { cat: MenuCategory; onToggle: () => void; onAddItem: () => void; onRemoveItem: (i: number) => void; onChangeItem: (i: number, field: 'name' | 'description', val: string) => void; }
...
const MinToggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, cursor: 'pointer', flexShrink: 0 }} onClick={e => { e.stopPropagation(); onChange(!checked); }}>
    <input type="checkbox" checked={checked} onChange={() => {}} style={{ position: 'absolute', opacity: 0, width: 0, height: 0 }} />
    <div style={{ width: 40, height: 22, position: 'relative', flexShrink: 0, background: checked ? ACCENT : RULE, transition: 'background .25s' }}>
      <div style={{ position: 'absolute', top: 2, left: 2, width: 18, height: 18, background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,.2)', transform: checked ? 'translateX(18px)' : 'translateX(0)', transition: 'transform .25s' }} />
    </div>
    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase', color: checked ? ACCENT : LIGHT, transition: 'color .25s', whiteSpace: 'nowrap' }}>
...
function CategoryCard({ cat, onToggle, onAddItem, onRemoveItem, onChangeItem }: CategoryCardProps) {
  ...
  return (
    <div style={{
      ...
      <div onClick={() => { if (cat.active) setExpanded(prev => !prev); }}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', cursor: cat.active ? 'pointer' : 'default', borderBottom: cat.active && expanded ? `1px solid ${RULE}` : '1px solid transparent', transition: 'border-color .25s', gap: 10, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
          <div style={{ width: 34, height: 34, background: cat.active ? 'rgba(200,80,58,.07)' : BG, border: `1px solid ${cat.active ? 'rgba(200,80,58,.2)' : RULE}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 14, transition: 'all .25s' }}>
            {cat.emoji}
          </div>
          <div style={{ minWidth: 0, flex: 1 }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(10px,2.5vw,12px)', fontWeight: 500, letterSpacing: '.04em', color: cat.active ? DARK : LIGHT, marginBottom: 2, transition: 'color .25s', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {cat.label}
            </p>
            {cat.active && cat.items.length > 0 && (
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 11, fontStyle: 'italic', color: LIGHT, marginBottom: 0 }}>{cat.items.length} {cat.items.length === 1 ? 'element' : 'elemente'}</p>
            )}
            {!cat.active && (
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 11, fontStyle: 'italic', color: '#CCCCCC', marginBottom: 0 }}>Activează pentru a configura</p>
            )}
          </div>
        </div>
      ...
      {cat.active && expanded && (
        <div style={{ padding: '12px 14px', animation: 'mn-fade-in .3s ease both' }}>
          {cat.items.length > 0 && (
            <div className="menu-col-labels" style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr 34px', gap: 8, marginBottom: 5, padding: '0 2px' }}>
              {['Nume', 'Descriere (opțional)', ''].map((h, i) => (
                <span key={i} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 7, letterSpacing: '.22em', textTransform: 'uppercase', color: LIGHT }}>{h}</span>
              ))}
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {cat.items.map((item, idx) => (
              <div key={idx} className="menu-item-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr 34px', gap: 8, alignItems: 'center', padding: '8px 10px', background: BG, border: `1px solid ${RULE}` }}>
                <input className="mn-inp" placeholder="ex: Somon afumat" value={item.name} onChange={e => onChangeItem(idx, 'name', e.target.value)} style={inputStyle} />
                <input className="mn-inp" placeholder="ex: cu cremă de avocado" value={item.description} onChange={e => onChangeItem(idx, 'description', e.target.value)} style={inputStyle} />
                <button type="button" className="mn-del-btn" onClick={() => onRemoveItem(idx)} style={{ width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', border: `1px solid ${RULE}`, cursor: 'pointer', color: LIGHT, transition: 'all .18s', flexShrink: 0 }}>
                  <IconTrash />
                </button>
              </div>
            ))}
          ...
          {cat.items.length === 0 && (
            <div style={{ padding: '16px 0', textAlign: 'center', border: `1px dashed ${RULE}`, marginBottom: 8 }}>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 13, fontStyle: 'italic', color: LIGHT, marginBottom: 0 }}>Niciun element adăugat</p>
            </div>
          )}
          <button type="button" className="mn-add-btn" onClick={onAddItem} style={{
            width: '100%', padding: '9px 0', marginTop: cat.items.length > 0 ? 8 : 0,
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
            background: 'transparent', border: `1px dashed rgba(200,80,58,.3)`, color: 'rgba(200,80,58,.6)',
            fontFamily: "'DM Sans', sans-serif", fontSize: 8, fontWeight: 600, letterSpacing: '.2em', textTransform: 'uppercase',
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .mn-inp { font-size: 16px !important; -webkit-text-size-adjust: 100%; -webkit-appearance: none; appearance: none; }
        .menu-section-wrap { overscroll-behavior: contain; -webkit-overflow-scrolling: auto; }
        .mn-inp:focus { border-color: #C8503A !important; outline: none; }
        .mn-inp::placeholder { color: #CCCCCC !important; font-style: italic; }
        .mn-del-btn:hover { background: rgba(200,80,58,.08) !important; border-color: rgba(200,80,58,.3) !important; color: #C8503A !important; }
        .mn-add-btn:hover { background: rgba(200,80,58,.05) !important; border-color: rgba(200,80,58,.5) !important; color: #C8503A !important; }
        ...
        @media (max-width: 600px) {
          ...
          .menu-item-row { grid-template-columns: 1fr 34px !important; grid-template-rows: auto auto; gap: 5px !important; }
      ...
      <div className="menu-section-wrap" style={{ animation: 'mn-fade-in .5s ease both', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, letterSpacing: '.3em', textTransform: 'uppercase', color: LIGHT }}>Configurare</p>
          </div>
            ...
            {isActive && (
              <div className="menu-stats-row" style={{ display: 'flex', gap: 16, flexShrink: 0 }}>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(18px,4vw,26px)', fontWeight: 400, fontStyle: 'italic', color: ACCENT, lineHeight: 1, marginBottom: 2 }}>{activeCount}</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 7, letterSpacing: '.18em', textTransform: 'uppercase', color: LIGHT, marginBottom: 0 }}>Categorii</p>
                </div>
                <div style={{ width: 1, background: RULE }} />
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(18px,4vw,26px)', fontWeight: 400, fontStyle: 'italic', color: ACCENT, lineHeight: 1, marginBottom: 2 }}>{totalItems}</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 7, letterSpacing: '.18em', textTransform: 'uppercase', color: LIGHT, marginBottom: 0 }}>Feluri</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{
          ...
          background: '#fff', border: `1px solid ${isActive ? 'rgba(200,80,58,.25)' : RULE}`,
          borderLeft: `3px solid ${isActive ? ACCENT : RULE}`,
          boxShadow: '0 2px 8px rgba(0,0,0,.04)', transition: 'all .25s',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
              <div style={{ width: 36, height: 36, background: isActive ? 'rgba(200,80,58,.08)' : BG, border: `1px solid ${isActive ? 'rgba(200,80,58,.2)' : RULE}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16, transition: 'all .25s' }}>🍽️</div>
              <div style={{ minWidth: 0 }}>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(10px,2.5vw,13px)', fontWeight: 500, letterSpacing: '.04em', color: DARK, marginBottom: 2 }}>Afișează meniul pe invitație</p>
        ...
        {isActive && (
          <div style={{ animation: 'mn-fade-in .35s ease both' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 8, letterSpacing: '.24em', textTransform: 'uppercase', color: LIGHT, marginBottom: 0 }}>Categorii Meniu</p>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontStyle: 'italic', color: LIGHT, marginBottom: 0 }}>Activează categoriile dorite &amp; adaugă felurile</p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
              {categories.map(cat => (
                <CategoryCard key={cat.id} cat={cat}
                  ...
                  onChangeItem={(i, field, val) => updateCat(cat.id, c => { const items = [...c.items]; items[i] = { ...items[i], [field]: val }; return { ...c, items }; })}
                />
              ))}
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, padding: '11px 14px', background: '#fff', border: `1px solid ${RULE}`, borderLeft: `3px solid rgba(200,80,58,.3)`, marginBottom: 16 }}>
        ...
        {/* SAVE */}
        <button type="button" className="mn-save-btn" onClick={handleSave} disabled={loading} style={{
          width: '100%', padding: '15px 0',
          background: DARK, color: '#fff',
          fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(10px,2.5vw,12px)', fontWeight: 600,
...
const inputStyle: React.CSSProperties = {
  ...
  minWidth: 0, boxSizing: 'border-box', WebkitAppearance: 'none',
};
