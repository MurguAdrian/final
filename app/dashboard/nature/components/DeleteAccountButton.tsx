  ...
  const modal = (
    ...
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{
          background: 'linear-gradient(160deg,#FDFAF2,#EDF5E8)',
      ...
      >
        {/* Warn icon */}
        <div style={{
          width: 48, height: 48, borderRadius: 14, margin: `0 auto ${SP.lg}px`,
          background: 'rgba(200,80,60,.08)', border: '1px solid rgba(200,80,60,.2)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <svg viewBox="0 0 24 24" fill="none" width={IS.xl} height={IS.xl}>
            <path
        ...
        <h3 style={{
          fontFamily: F.heading, fontSize: 18, fontWeight: 500, color: '#1C2218',
          textAlign: 'center', margin: `0 0 10px`, letterSpacing: '.06em',
        }}>
          Șterge Contul
        </h3>
        ...
        <p style={{
          fontSize: FS.md, color: 'rgba(28,34,24,.65)',
          textAlign: 'center', lineHeight: 1.75, margin: `0 0 ${SP.sm}px`,
        }}>
          Această acțiune este{' '}
          <strong style={{ color: 'rgba(180,60,40,.85)' }}>permanentă și ireversibilă.</strong>
          {' '}Se vor șterge invitația, confirmările, pozele și toate setările.
        ...
        <p style={{
          fontSize: FS.md - 2, color: 'rgba(180,60,40,.7)',
          textAlign: 'center', lineHeight: 1.6, margin: `0 0 ${SP.xl}px`, fontStyle: 'italic',
        }}>
          Nu vei primi nicio rambursare pentru suma plătită.
        </p>
        ...
        <label style={{
          ...
          background: 'rgba(200,80,60,.04)',
          border: `1px solid ${confirmed ? 'rgba(200,80,60,.35)' : 'rgba(200,80,60,.15)'}`,
          borderRadius: BR.md, padding: `10px 13px`, transition: 'border-color .2s',
        }}>
          <input
            type="checkbox"
            checked={confirmed}
            onChange={e => setConfirmed(e.target.checked)}
            style={{ marginTop: 2, accentColor: '#3A5E33', width: 15, height: 15, flexShrink: 0 }}
          />
          <span style={{ fontSize: FS.md, color: 'rgba(28,34,24,.75)', lineHeight: 1.55 }}>
            Înțeleg că datele mele vor fi șterse definitiv și că nu voi primi banii înapoi.
          </span>
        </label>
        ...
        <div style={{ display: 'flex', gap: SP.sm }}>
          <button
            ...
            disabled={loading}
            style={{
              flex: 1, padding: '10px 14px', borderRadius: BR.md,
              background: 'rgba(58,94,51,.07)', border: '1px solid rgba(58,94,51,.18)',
              color: 'rgba(58,94,51,.75)', fontFamily: F.heading,
              fontSize: FS.tiny, fontWeight: 600, letterSpacing: '.14em',
          ...
          <button
            ...
            style={{
              ...
              background: confirmed ? 'rgba(190,60,40,.82)' : 'rgba(200,80,60,.15)',
              border: '1px solid rgba(200,80,60,.25)',
              color: confirmed ? C.whiteSoft : 'rgba(200,80,60,.35)',
  ...
  return (
    <>
      <button
        ...
        style={{
          background: 'none', border: 'none',
          cursor: 'pointer', padding: SP.sm - 2, borderRadius: BR.sm,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: 'rgba(180,60,40,.5)', flexShrink: 0,
        }}
        onMouseEnter={e => {
          e.currentTarget.style.color = 'rgba(180,60,40,.9)';
          e.currentTarget.style.background = 'rgba(200,80,60,.08)';
        }}
        onMouseLeave={e => {
          e.currentTarget.style.color = 'rgba(180,60,40,.5)';
          e.currentTarget.style.background = 'none';
        }}
      >
        <svg viewBox="0 0 20 20" fill="none" width={IS.md} height={IS.md}>
          <path
            d="M4 6h12M9 6V4h2v2M8 9v6M12 9v6M5 6l1 10h8l1-10"
            stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
          />
        </svg>
      </button>
