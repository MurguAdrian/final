...
export const SummarySection = ({ isComplete }: SummaryProps) => {
  ...
  if (loading) return (
    <div style={{
      ...
      <div style={{
        width: 18, height: 18,
        border: '1.5px solid rgba(58,94,51,.25)',
        borderTopColor: '#3A5E33',
        borderRadius: '50%',
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .rm-row:hover        { background: rgba(58,94,51,.03) !important; }
        .rm-export-btn:hover { background: rgba(58,94,51,.1) !important; border-color: rgba(58,94,51,.45) !important; color: #274422 !important; }
        .rm-copy-btn:hover   { background: linear-gradient(135deg,#274422,#3A5E33,#5C8A52,#3A5E33,#274422) !important; }
        .rm-share-btn:hover  { background: rgba(58,94,51,.1) !important; border-color: rgba(58,94,51,.45) !important; color: #274422 !important; }
      ...
      <div className="sum-wrap" style={{ animation: 'rm-fade-in .55s ease both', fontFamily: "'Cormorant Garamond', serif" }}>
        ...
        <div className="sum-header">
          <div>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 9, letterSpacing: '.36em', textTransform: 'uppercase', color: 'rgba(58,94,51,.45)', marginBottom: 8 }}>
              Panou Principal
            </p>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(22px,4vw,38px)', fontWeight: 300, fontStyle: 'italic', color: '#1C2218', margin: 0, lineHeight: 1.1 }}>
              Centrul de Comandă
            </h2>
          </div>
          {data?.guests?.length > 0 && (
            <button className="rm-export-btn" onClick={exportToExcel} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 4, background: 'rgba(58,94,51,.05)', border: '1px solid rgba(58,94,51,.22)', color: 'rgba(58,94,51,.75)', fontFamily: "'Cinzel', serif", fontSize: 10, fontWeight: 600, letterSpacing: '.16em', textTransform: 'uppercase', cursor: 'pointer', transition: 'all .2s', whiteSpace: 'nowrap', flexShrink: 0 }}>
              <svg viewBox="0 0 20 20" fill="none" style={{ width: 13, height: 13, flexShrink: 0 }}>
                <path d="M10 13V4M6 9l4 4 4-4M4 16h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span>Exportă Lista</span>
        ...
        {/* LINK CARD */}
        <div style={{ borderRadius: 14, overflow: 'hidden', border: `1px solid ${isComplete ? 'rgba(58,94,51,.25)' : 'rgba(255,165,0,.3)'}`, background: isComplete ? 'linear-gradient(160deg,rgba(58,94,51,.05) 0%,rgba(58,94,51,.02) 100%)' : 'linear-gradient(160deg,rgba(255,140,0,.08) 0%,rgba(255,140,0,.03) 100%)', padding: 'clamp(16px,3vw,24px)', marginBottom: 28, position: 'relative', boxShadow: '0 4px 24px rgba(26,38,20,.06),inset 0 1px 0 rgba(58,94,51,.06)' }}>
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,${isComplete ? 'rgba(58,94,51,.35)' : 'rgba(255,165,0,.25)'},transparent)` }} />
          ...
          {!isComplete ? (
            <div>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(13px,2vw,16px)', fontStyle: 'italic', color: 'rgba(28,34,24,.45)', lineHeight: 1.7 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, background: 'rgba(58,94,51,.1)', border: '1px solid rgba(58,94,51,.28)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg viewBox="0 0 24 24" fill="#3A5E33" style={{ width: 14, height: 14 }}>
                    <path d="M17 8C8 10 5.9 16.17 3.82 19.5c-.6.95.26 2.04 1.3 1.72C7.27 20.42 11.5 19 14 16c3-3.5 4-9 3-8z" />
                  </svg>
                </div>
                <h4 style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(10px,2.5vw,13px)', fontWeight: 600, letterSpacing: '.1em', color: '#3A5E33', margin: 0 }}>
...
setTimeout(() => {
  ...
  setTimeout(() => document.body.removeChild(el), 250);
        ...
        {/* STATS GRID */}
        <div className="sum-stats-grid">
          <StatCard title="Vizualizări" value={data?.views} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M1 10s4-6 9-6 9 6 9 6-4 6-9 6-9-6-9-6z" stroke="#3A5E33" strokeWidth="1.3" strokeLinecap="round" /><circle cx="10" cy="10" r="2.5" stroke="#3A5E33" strokeWidth="1.3" /></svg>} />
          <StatCard title="Confirmări (DA)" value={data?.stats?.da} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M4 10l4 4 8-8" stroke="#3A5E33" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>} />
          <StatCard title="Total Persoane" value={(data?.stats?.adulti || 0) + (data?.stats?.copii || 0)} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><circle cx="8" cy="6" r="2.5" stroke="#3A5E33" strokeWidth="1.3" /><path d="M3 17c0-3 2-5 5-5s5 2 5 5" stroke="#3A5E33" strokeWidth="1.3" strokeLinecap="round" /><path d="M13 8c1.3.6 2 1.8 2 3M16 17c0-2.5-1-4.5-3-5.5" stroke="#3A5E33" strokeWidth="1.3" strokeLinecap="round" /></svg>} />
          <StatCard title="Cazare" value={data?.stats?.cazare} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M3 17V8l7-5 7 5v9M8 17v-5h4v5" stroke="#3A5E33" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" /></svg>} />
          <StatCard title="Transport" value={data?.stats?.transport} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><rect x="2" y="7" width="16" height="9" rx="2" stroke="#3A5E33" strokeWidth="1.3" /><path d="M5 7V5a3 3 0 0 1 6 0v2M6 16v2M14 16v2" stroke="#3A5E33" strokeWidth="1.3" strokeLinecap="round" /></svg>} />
        </div>

        <BotanicalDivider />

        {/* GUEST TABLE */}
        <div style={{ background: 'rgba(58,94,51,.02)', border: '1px solid rgba(58,94,51,.15)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(26,38,20,.05),inset 0 1px 0 rgba(58,94,51,.05)', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(58,94,51,.35),transparent)' }} />

          <div style={{ padding: 'clamp(16px,3vw,24px)', borderBottom: '1px solid rgba(58,94,51,.1)' }}>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 8, letterSpacing: '.32em', textTransform: 'uppercase', color: 'rgba(58,94,51,.4)', marginBottom: 5 }}>Registrul Invitaților</p>
            <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(16px,3vw,24px)', fontStyle: 'italic', fontWeight: 300, color: '#1C2218', margin: 0 }}>
              Detalii Răspunsuri
              {data?.guests?.length > 0 && (
                <span style={{ fontFamily: "'Cinzel', serif", fontSize: 9, letterSpacing: '.18em', color: 'rgba(58,94,51,.4)', marginLeft: 12, fontStyle: 'normal', fontWeight: 400 }}>
          ...
          <div className="sum-table-wrap" ref={tableWrapRef}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 400 }}>
              ...
              <tbody>
                {data?.guests?.length > 0 ? (
                  data.guests.map((guest: any) => (
                    <tr key={guest.id} className="rm-row" style={{ borderBottom: '1px solid rgba(58,94,51,.06)', transition: 'background .2s' }}>
                      <td style={tdStyle}>
                        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(13px,2vw,17px)', fontWeight: 600, color: '#1C2218' }}>{guest.guest_name}</span>
                        {guest.partner_name && <span style={{ display: 'block', fontFamily: "'Cormorant Garamond', serif", fontSize: 11, fontStyle: 'italic', color: 'rgba(58,94,51,.4)', marginTop: 2 }}>+ {guest.partner_name}</span>}
                      </td>
                      <td style={tdStyle}>
                        {guest.is_coming
                      ...
                      <td style={tdStyle}><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: 'rgba(58,94,51,.65)', letterSpacing: '.06em' }}>{guest.adults_count}A / {guest.kids_count}C</span></td>
                      <td style={tdStyle} className="td-cazare"><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: guest.needs_accommodation ? 'rgba(58,94,51,.8)' : 'rgba(28,34,24,.2)', letterSpacing: '.06em' }}>{guest.needs_accommodation ? "🌿 DA" : "—"}</span></td>
                      <td style={tdStyle} className="td-transport"><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: guest.needs_transport ? 'rgba(58,94,51,.8)' : 'rgba(28,34,24,.2)', letterSpacing: '.06em' }}>{guest.needs_transport ? "🌿 DA" : "—"}</span></td>
                      <td style={tdStyle} className="td-details">
                        {guest.dietary_preferences && <div style={{ marginBottom: guest.other_mentions ? 5 : 0 }}><span style={{ fontFamily: "'Cinzel', serif", fontSize: 7, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(58,94,51,.45)', display: 'block', marginBottom: 2 }}>Dietă</span><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 13, fontStyle: 'italic', color: 'rgba(28,34,24,.6)' }}>{guest.dietary_preferences}</span></div>}
                        {guest.other_mentions     && <div><span style={{ fontFamily: "'Cinzel', serif", fontSize: 7, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(58,94,51,.45)', display: 'block', marginBottom: 2 }}>Mesaj</span><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 13, fontStyle: 'italic', color: 'rgba(28,34,24,.6)' }}>{guest.other_mentions}</span></div>}
                        {!guest.dietary_preferences && !guest.other_mentions && <span style={{ color: 'rgba(28,34,24,.18)', fontSize: 14 }}>—</span>}
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={6} style={{ padding: 'clamp(32px,5vw,56px)', textAlign: 'center' }}>
                      ...
                      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, fontStyle: 'italic', fontWeight: 300, color: 'rgba(58,94,51,.3)', marginBottom: 5 }}>Niciun răspuns încă</p>
                      <p style={{ fontFamily: "'Cinzel', serif", fontSize: 8, letterSpacing: '.22em', textTransform: 'uppercase', color: 'rgba(58,94,51,.22)' }}>Distribuie invitația pentru a primi confirmări</p>
                    </td>
                  </tr>
                )}
              </tbody>
...
const thStyle: React.CSSProperties = {
  ...
  textAlign: 'left',
  fontWeight: 600,
  borderBottom: '1px solid rgba(58,94,51,.12)',
  whiteSpace: 'nowrap',
  position: 'sticky',
  top: 0,
  background: '#F5FAF3',
...
const StatCard = ({ title, value, icon }: any) => (
  <div
    style={{
      background: 'rgba(58,94,51,.03)',
      border: '1px solid rgba(58,94,51,.15)',
      borderRadius: 14,
      padding: 'clamp(12px,2vw,20px) clamp(10px,1.5vw,16px)',
      textAlign: 'center' as const,
      position: 'relative',
      overflow: 'hidden',
      boxShadow: '0 2px 16px rgba(26,38,20,.04), inset 0 1px 0 rgba(58,94,51,.06)',
    }}
  >
    <div
      style={{
        ...
        right: '15%',
        height: 1,
        background: 'linear-gradient(90deg,transparent,rgba(58,94,51,.3),transparent)',
      }}
    />
...
const BotanicalDivider = () => (
  <div style={{ display: 'flex', alignItems: 'center', width: '100%', marginBottom: 24 }}>
    ...
    <svg viewBox="0 0 60 20" width="54" height="18" fill="none" style={{ flexShrink: 0 }}>
      <path d="M5 10 L20 10" stroke="#3A5E33" strokeWidth=".8" strokeOpacity=".5" />
      <path d="M40 10 L55 10" stroke="#3A5E33" strokeWidth=".8" strokeOpacity=".5" />
      <path d="M30 4 Q34 7 34 10 Q34 13 30 16 Q26 13 26 10 Q26 7 30 4Z" fill="none" stroke="#3A5E33" strokeWidth="1" strokeOpacity=".8" />
      <circle cx="30" cy="10" r="2" fill="#3A5E33" fillOpacity=".7" />
      <circle cx="18" cy="10" r="1" fill="#3A5E33" fillOpacity=".4" />
      <circle cx="42" cy="10" r="1" fill="#3A5E33" fillOpacity=".4" />
    </svg>
