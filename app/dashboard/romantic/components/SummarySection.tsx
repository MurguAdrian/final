  ...
  */
  ...
  if (loading) return (
    <div style={{
      ...
      <div style={{
        width: 18, height: 18,
        border: '1.5px solid rgba(196,80,106,.25)',
        borderTopColor: '#A63248',
        borderRadius: '50%',
    ...
    <>
      ...
      <div className="sum-wrap" style={{ animation: 'rm-fade-in .55s ease both', fontFamily: "'Cormorant Garamond', serif" }}>
        ...
        <div className="sum-header">
          <div>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 9, letterSpacing: '.36em', textTransform: 'uppercase', color: 'rgba(166,50,72,.45)', marginBottom: 8 }}>
              Panou Principal
            </p>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(22px,4vw,38px)', fontWeight: 300, fontStyle: 'italic', color: '#3D1520', margin: 0, lineHeight: 1.1 }}>
              Centrul de Comandă
            </h2>
          </div>
          {data?.guests?.length > 0 && (
            <button className="rm-export-btn" onClick={exportToExcel} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 4, background: 'rgba(196,80,106,.05)', border: '1px solid rgba(196,80,106,.22)', color: 'rgba(166,50,72,.75)', fontFamily: "'Cinzel', serif", fontSize: 10, fontWeight: 600, letterSpacing: '.16em', textTransform: 'uppercase', cursor: 'pointer', transition: 'all .2s', whiteSpace: 'nowrap', flexShrink: 0 }}>
        ...
        {/* LINK CARD */}
        <div style={{ borderRadius: 14, overflow: 'hidden', border: `1px solid ${isComplete ? 'rgba(196,80,106,.25)' : 'rgba(255,165,0,.3)'}`, background: isComplete ? 'linear-gradient(160deg,rgba(196,80,106,.05) 0%,rgba(196,80,106,.02) 100%)' : 'linear-gradient(160deg,rgba(255,140,0,.08) 0%,rgba(255,140,0,.03) 100%)', padding: 'clamp(16px,3vw,24px)', marginBottom: 28, position: 'relative', boxShadow: '0 4px 24px rgba(123,26,46,.06),inset 0 1px 0 rgba(196,80,106,.06)' }}>
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,${isComplete ? 'rgba(196,80,106,.35)' : 'rgba(255,165,0,.25)'},transparent)` }} />
          ...
          {!isComplete ? (
            <div>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(13px,2vw,16px)', fontStyle: 'italic', color: 'rgba(61,21,32,.45)', lineHeight: 1.7 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, background: 'rgba(196,80,106,.1)', border: '1px solid rgba(196,80,106,.28)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <h4 style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(10px,2.5vw,13px)', fontWeight: 600, letterSpacing: '.1em', color: '#A63248', margin: 0 }}>
              ...
              <div className="sum-link-row">
                ...
                />
                <button className="rm-copy-btn" onClick={() => { navigator.clipboard.writeText(`https://www.vibeinvite.ro/invitatie/romantic/${userSlug}`); const el = document.createElement('div');
...
el.style.cssText = `
  ...
  background: linear-gradient(145deg, #2a0d12 0%, #14070a 100%);
...
setTimeout(() => {
  ...
  setTimeout(() => document.body.removeChild(el), 250);
}, 1600); }} style={{ padding: '10px 20px', borderRadius: 8, background: 'rgba(196,80,106,.12)', border: '1px solid rgba(196,80,106,.3)', color: '#7B1A2E', fontFamily: "'Cinzel', serif", fontSize: 16, fontWeight: 700, letterSpacing: '.18em', textTransform: 'uppercase' as const, cursor: 'pointer', transition: 'all .2s', whiteSpace: 'nowrap' as const }}>
                  Copiază
                </button>
        ...
        <div className="sum-stats-grid">
          <StatCard title="Vizualizări" value={data?.views} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M1 10s4-6 9-6 9 6 9 6-4 6-9 6-9-6-9-6z" stroke="#A63248" strokeWidth="1.3" strokeLinecap="round" /><circle cx="10" cy="10" r="2.5" stroke="#A63248" strokeWidth="1.3" /></svg>} />
          <StatCard title="Confirmări (DA)" value={data?.stats?.da} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M4 10l4 4 8-8" stroke="#A63248" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>} />
          <StatCard title="Total Persoane" value={(data?.stats?.adulti || 0) + (data?.stats?.copii || 0)} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><circle cx="8" cy="6" r="2.5" stroke="#A63248" strokeWidth="1.3" /><path d="M3 17c0-3 2-5 5-5s5 2 5 5" stroke="#A63248" strokeWidth="1.3" strokeLinecap="round" /><path d="M13 8c1.3.6 2 1.8 2 3M16 17c0-2.5-1-4.5-3-5.5" stroke="#A63248" strokeWidth="1.3" strokeLinecap="round" /></svg>} />
          <StatCard title="Cazare" value={data?.stats?.cazare} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><path d="M3 17V8l7-5 7 5v9M8 17v-5h4v5" stroke="#A63248" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" /></svg>} />
          <StatCard title="Transport" value={data?.stats?.transport} icon={<svg viewBox="0 0 20 20" fill="none" style={{ width: 15, height: 15 }}><rect x="2" y="7" width="16" height="9" rx="2" stroke="#A63248" strokeWidth="1.3" /><path d="M5 7V5a3 3 0 0 1 6 0v2M6 16v2M14 16v2" stroke="#A63248" strokeWidth="1.3" strokeLinecap="round" /></svg>} />
        </div>
              ...
              <tbody>
                {data?.guests?.length > 0 ? (
                  data.guests.map((guest: any) => (
                    <tr key={guest.id} className="rm-row" style={{ borderBottom: '1px solid rgba(196,80,106,.06)', transition: 'background .2s' }}>
                      <td style={tdStyle}>
                        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(13px,2vw,17px)', fontWeight: 600, color: '#3D1520' }}>{guest.guest_name}</span>
                        {guest.partner_name && <span style={{ display: 'block', fontFamily: "'Cormorant Garamond', serif", fontSize: 11, fontStyle: 'italic', color: 'rgba(166,50,72,.4)', marginTop: 2 }}>+ {guest.partner_name}</span>}
                      </td>
                      <td style={tdStyle}>
                        {guest.is_coming
                      ...
                      <td style={tdStyle}><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: 'rgba(166,50,72,.65)', letterSpacing: '.06em' }}>{guest.adults_count}A / {guest.kids_count}C</span></td>
                      <td style={tdStyle} className="td-cazare"><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: guest.needs_accommodation ? 'rgba(166,50,72,.8)' : 'rgba(61,21,32,.2)', letterSpacing: '.06em' }}>{guest.needs_accommodation ? "♥ DA" : "—"}</span></td>
                      <td style={tdStyle} className="td-transport"><span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: guest.needs_transport ? 'rgba(166,50,72,.8)' : 'rgba(61,21,32,.2)', letterSpacing: '.06em' }}>{guest.needs_transport ? "♥ DA" : "—"}</span></td>
                      <td style={tdStyle} className="td-details">
                        {guest.dietary_preferences && <div style={{ marginBottom: guest.other_mentions ? 5 : 0 }}><span style={{ fontFamily: "'Cinzel', serif", fontSize: 7, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(166,50,72,.45)', display: 'block', marginBottom: 2 }}>Dietă</span><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 13, fontStyle: 'italic', color: 'rgba(61,21,32,.6)' }}>{guest.dietary_preferences}</span></div>}
                        {guest.other_mentions     && <div><span style={{ fontFamily: "'Cinzel', serif", fontSize: 7, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(166,50,72,.45)', display: 'block', marginBottom: 2 }}>Mesaj</span><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 13, fontStyle: 'italic', color: 'rgba(61,21,32,.6)' }}>{guest.other_mentions}</span></div>}
                        {!guest.dietary_preferences && !guest.other_mentions && <span style={{ color: 'rgba(61,21,32,.18)', fontSize: 14 }}>—</span>}
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={6} style={{ padding: 'clamp(32px,5vw,56px)', textAlign: 'center' }}>
                      ...
                      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, fontStyle: 'italic', fontWeight: 300, color: 'rgba(166,50,72,.3)', marginBottom: 5 }}>Niciun răspuns încă</p>
                      <p style={{ fontFamily: "'Cinzel', serif", fontSize: 8, letterSpacing: '.22em', textTransform: 'uppercase', color: 'rgba(166,50,72,.22)' }}>Distribuie invitația pentru a primi confirmări</p>
                    </td>
                  </tr>
                )}
              </tbody>
