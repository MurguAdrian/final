// FIȘIER: andre/app/dashboard/royal/components/MenuSection.tsx
// MODIFICĂRI FAȚĂ DE ROMANTIC:
//   - Import tokens: romanticTokens → royalTokens
//   - Culori: rgba(196,80,106,...) → rgba(26,48,96,...), C.crimson/rose/peony/blush → navy/royal
//   - HeartSVG → CrownSVG (icon regal, dimensiuni identice)
//   - RoseDivider → RoyalDivider (același layout, culori navy/silver)
//   - RomanticToggle → RoyalToggle (același layout, culori navy)
...
const CrownSVG = ({ size = IS.md, color = C.crimson }: { size?: number; color?: string }) => (
  <svg viewBox="0 0 24 18" fill={color} style={{ width: size, height: size * 18 / 24, flexShrink: 0 }}>
  ...
  <div style={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0, margin: `${SP.xl}px 0` }}>
    ...
    <svg viewBox="0 0 80 30" width="80" height="30" fill="none">
      ...
      <g transform="translate(40 15)">
        <rect x="-8" y="-8" width="16" height="16" transform="rotate(45)" fill="none" stroke={C.peony} strokeWidth="1" strokeOpacity=".6" />
        <rect x="-4" y="-4" width="8" height="8" transform="rotate(45)" fill={C.peony} fillOpacity=".3" />
        <circle cx="0" cy="0" r="2.5" fill={C.crimson} fillOpacity=".75" />
        <circle cx="0" cy="0" r="1" fill={C.gold} fillOpacity=".7" />
      </g>
...
/* ══════════════════════════════════════════════════════════════
   ROYAL TOGGLE
══════════════════════════════════════════════════════════════ */
const RoyalToggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
  <label
  ...
  >
    ...
    <div style={{
      ...
      background: checked ? GR.roseBtn : 'rgba(26,48,96,.1)',
      border: `1px solid ${checked ? 'rgba(26,48,96,.5)' : 'rgba(26,48,96,.18)'}`,
      boxShadow: checked ? SH.toggleGlow : 'none',
      transition: 'all .3s ease',
    }}>
      <div style={{
        position: 'absolute', top: 2, left: 2, width: 19, height: 19, borderRadius: '50%',
        background: checked ? C.white : `rgba(26,48,96,.35)`,
        boxShadow: '0 2px 6px rgba(11,25,41,.3)',
        transform: checked ? 'translateX(21px)' : 'translateX(0)',
        transition: 'transform .3s ease, background .3s ease',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        {checked && <CrownSVG size={9} color={C.crimson} />}
      </div>
    </div>
    <span style={{
      fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.16em', textTransform: 'uppercase' as const,
      color: checked ? C.crimson : `rgba(58,84,120,.4)`,
...
function CategoryCard({ cat, onToggle, onAddItem, onRemoveItem, onChangeItem }: CategoryCardProps) {
  ...
  return (
    <div style={{
      borderRadius: BR.xxl,
      overflow: 'hidden',
      border: `1.5px solid ${cat.active ? 'rgba(26,48,96,.28)' : 'rgba(26,48,96,.1)'}`,
      background: cat.active ? GR.cardActive : GR.cardInactive,
      boxShadow: cat.active ? SH.cardActive : `0 2px 10px rgba(11,25,41,.04)`,
      ...
      <div
        ...
        style={{
          ...
          borderBottom: cat.active && expanded ? `1px solid rgba(26,48,96,.14)` : '1px solid transparent',
      ...
      >
        {cat.active && (
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,rgba(26,48,96,.3),transparent)` }} />
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: SP.sm, flex: 1, minWidth: 0 }}>
          <div style={{
            width: 40, height: 40, borderRadius: BR.lg, flexShrink: 0,
            background: cat.active ? `linear-gradient(135deg,rgba(26,48,96,.14) 0%,rgba(232,240,248,.6) 100%)` : 'rgba(26,48,96,.06)',
            border: `1px solid ${cat.active ? 'rgba(26,48,96,.28)' : 'rgba(26,48,96,.12)'}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <p style={{
              ...
              letterSpacing: '.08em', color: cat.active ? C.dark : `rgba(58,84,120,.35)`,
            ...
            {!cat.active && (
              <p style={{ fontFamily: F.body, fontSize: FS.base, fontStyle: 'italic', color: `rgba(26,48,96,.3)`, marginBottom: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: SP.sm, flexShrink: 0 }}>
          <RoyalToggle checked={cat.active} onChange={onToggle} />
          {cat.active && (
            <span style={{ color: `rgba(26,48,96,.55)`, transition: 'color .2s' }}>
      ...
      {cat.active && expanded && (
        <div style={{ padding: `14px ${SP.lg}px`, animation: 'rm-fade-in .35s ease both' }}>
          {cat.items.length > 0 && (
            ...
            >
              {['Nume', 'Descriere (opțional)', ''].map((h, i) => (
                <span key={i} style={{ fontFamily: F.heading, fontSize: 7, letterSpacing: '.22em', textTransform: 'uppercase' as const, color: `rgba(58,84,120,.5)` }}>
                  {h}
                </span>
              ))}
            </div>
            ...
            {cat.items.map((item, idx) => (
              <div
                ...
                style={{
                  ...
                  background: `rgba(244,247,250,.75)`,
                  border: `1px solid rgba(26,48,96,.14)`,
                  boxShadow: `0 2px 8px rgba(11,25,41,.04)`,
                }}
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: 'rgba(244,247,250,.8)',
                    border: `1px solid rgba(26,48,96,.18)`,
                    borderRadius: BR.md, cursor: 'pointer',
                    color: `rgba(26,48,96,.5)`, transition: 'all .2s', flexShrink: 0,
                  }}
                >
                  <IconTrash />
                </button>
          ...
          {cat.items.length === 0 && (
            <div style={{ padding: `${SP.xl}px 0`, textAlign: 'center', border: `1px dashed rgba(26,48,96,.2)`, borderRadius: BR.lg, marginBottom: SP.sm }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 6 }}>
                <CrownSVG size={IS.md} color={`rgba(26,48,96,.3)`} />
              </div>
              <p style={{ fontFamily: F.body, fontSize: 14, fontStyle: 'italic', fontWeight: 300, color: `rgba(58,84,120,.4)`, marginBottom: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              border: `1px dashed rgba(26,48,96,.3)`, borderRadius: BR.lg,
              color: `rgba(26,48,96,.6)`,
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .rm-inp:focus        { border-color: rgba(26,48,96,.55) !important; background: rgba(232,240,248,.85) !important; outline: none; }
        .rm-inp::placeholder { color: rgba(58,84,120,.35) !important; font-style: italic; }

        .rm-del-btn:hover { background: rgba(26,48,96,.12) !important; border-color: rgba(26,48,96,.4) !important; color: ${C.crimson} !important; }
        .rm-add-btn:hover { background: rgba(232,240,248,.45) !important; border-color: rgba(26,48,96,.5) !important; color: ${C.crimson} !important; }
        .rm-save-btn:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 18px 50px rgba(11,25,41,.48) !important; }
      ...
      <div className="menu-section-wrap" style={{ animation: 'rm-fade-in .55s ease both', fontFamily: F.body, paddingBottom: '10vh' }}>
            ...
            {isActive && (
              <div className="menu-stats-row" style={{
                ...
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(18px,4vw,26px)', fontWeight: 400, fontStyle: 'italic', color: C.crimson, lineHeight: 1, marginBottom: 1 }}>{activeCount}</p>
                ...
                <CrownSVG size={10} color={`rgba(26,48,96,.35)`} />
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(18px,4vw,26px)', fontWeight: 400, fontStyle: 'italic', color: C.crimson, lineHeight: 1, marginBottom: 1 }}>{totalItems}</p>
        ...
        <div style={{
          ...
          background: isActive ? GR.masterActive : `rgba(244,247,250,.55)`,
          border: `1.5px solid ${isActive ? 'rgba(26,48,96,.28)' : 'rgba(26,48,96,.12)'}`,
          borderRadius: BR.xxl,
          boxShadow: isActive ? SH.cardActive : `0 4px 16px rgba(11,25,41,.04)`,
          backdropFilter: 'blur(10px)',
          position: 'relative', overflow: 'hidden', transition: 'all .3s',
        }}>
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,${isActive ? 'rgba(26,48,96,.45)' : 'rgba(26,48,96,.2)'},transparent)` }} />
          <div className="menu-master-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: BR.lg }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: BR.lg, flex: 1, minWidth: 0 }}>
              <div style={{
                ...
                background: isActive ? `linear-gradient(135deg,rgba(26,48,96,.16) 0%,rgba(232,240,248,.7) 100%)` : 'rgba(26,48,96,.07)',
                border: `1.5px solid ${isActive ? 'rgba(26,48,96,.3)' : 'rgba(26,48,96,.14)'}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ fontFamily: F.body, fontSize: FS.md, fontStyle: 'italic', color: isActive ? C.textLight : `rgba(58,84,120,.4)`, lineHeight: 1.4, marginBottom: 0 }}>
                  {isActive ? 'Invitații vor vedea meniul complet ✦' : 'Activează pentru a configura și afișa'}
                </p>
              </div>
            </div>
            <RoyalToggle checked={isActive} onChange={setIsActive} />
          </div>
        </div>
        ...
        {isActive && (
          <div style={{ animation: 'rm-fade-in .4s ease both' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap' as const, gap: SP.sm }}>
              ...
              <p style={{ fontFamily: F.body, fontSize: FS.md, fontStyle: 'italic', color: `rgba(58,84,120,.5)`, marginBottom: 0 }}>
            ...
            <div style={{
              ...
              background: `rgba(244,247,250,.7)`, border: `1px solid rgba(26,48,96,.15)`,
              backdropFilter: 'blur(6px)', marginBottom: SP.xl,
            }}>
              <div style={{ flexShrink: 0, marginTop: 1 }}>
                <CrownSVG size={IS.md - 1} color={`rgba(26,48,96,.45)`} />
              </div>
        ...
        >
          <span style={{ position: 'relative', zIndex: 1, display: 'inline-flex', alignItems: 'center', gap: SP.sm }}>
            {loading
              ...
              : <><CrownSVG size={IS.md - 1} color={C.white} /> Salvează Meniul <CrownSVG size={IS.md - 1} color={C.white} /></>
            }
          </span>
          {!loading && (
        ...
        <p style={{ fontFamily: F.heading, fontSize: FS.micro, letterSpacing: '.2em', textTransform: 'uppercase' as const, color: `rgba(58,84,120,.25)`, textAlign: 'center', marginTop: SP.lg }}>
          VibeInvite · Meniu Nuntă · Tema Royal
        </p>
      </div>
    </>
  );
