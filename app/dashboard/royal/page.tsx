// FIȘIER: andre/app/dashboard/royal/page.tsx
// MODIFICĂRI FAȚĂ DE ROMANTIC:
//   - Culori: rose/crimson → navy/royal
//   - Import tokens: romanticTokens → royalTokens
//   - Import paths componente: ./components/ → același dar fișierele royal
//   - Sidebar label: "Romantic Edition" → "Royal Edition"
//   - URL preview: /invitatie/romantic/ → /invitatie/royal/
//   - CSS rgba romantic → rgba royal navy
...
.rm-tablet-nav {
  ...
  background: rgba(244,247,250,0.97);
  border-bottom: 1px solid rgba(26,48,96,0.12);
...
.rm-mobile-nav {
  ...
  -webkit-backdrop-filter: blur(20px);
  backdrop-filter: blur(20px);
  box-shadow: 0 -4px 30px rgba(11,25,41,0.08);
  display: none;
...
.rm-tab:hover {
  background: rgba(26,48,96,0.07) !important;
  color: ${C.rose} !important;
}
.rm-tab-active {
  background: linear-gradient(135deg, rgba(26,48,96,0.12) 0%, rgba(26,48,96,0.05) 100%) !important;
  color: ${C.crimson} !important;
  border-color: rgba(26,48,96,0.3) !important;
}
...
.rm-mobile-tab:hover { background: rgba(26,48,96,0.06) !important; }
...
const CrownLogo = () => (
  <svg viewBox="0 0 24 18" fill={C.rose} style={{ width: 32, height: 24, margin: '0 auto', filter: 'drop-shadow(0 2px 8px rgba(26,48,96,.25))' }}>
    <path d="M2 16h20v2H2zM2 14l4-10 6 6 6-6 4 10H2z" />
  </svg>
);

const CrownIcon = ({ size = IS.md, color = C.rose }: { size?: number; color?: string }) => (
...
const LoadingScreen = () => (
  <>
    ...
    <div style={{
      ...
      <div style={{
        width: 36, height: 36,
        border: `1.5px solid rgba(26,48,96,0.2)`,
...
const PulseDot = ({ active }: { active: boolean }) => (
  <div style={{
    ...
    boxShadow: `0 0 ${active ? 8 : 6}px ${active ? 'rgba(26,48,96,0.6)' : 'rgba(255,165,0,0.6)'}`,
...
export default function RoyalDashboard() {
  ...
  const StatusBadge = ({ compact = false }: { compact?: boolean }) => (
    <div style={{
      ...
      background: compact
        ? (isProfileComplete ? 'rgba(26,48,96,0.07)' : 'rgba(255,165,0,0.08)')
        : undefined,
      border: compact
        ? `1px solid ${isProfileComplete ? 'rgba(26,48,96,0.25)' : 'rgba(255,165,0,0.25)'}`
        : undefined,
      fontFamily: F.heading, fontSize: FS.tiny, letterSpacing: '.12em',
      color: isProfileComplete ? 'rgba(26,48,96,0.85)' : 'rgba(255,165,0,0.8)',
      ...
      return (
        <button
          ...
          style={{
            ...
            color: isActive ? C.rose : 'rgba(26,48,96,0.35)',
          ...
          <span style={{
            ...
            color: isActive ? C.rose : 'rgba(26,48,96,0.3)',
      ...
      return (
        <button
          ...
          style={{
            ...
            color: isActive ? C.crimson : 'rgba(26,48,96,0.45)',
          ...
          {tab.label}
          {isActive && (
            <div style={{ width: 4, height: 4, borderRadius: '50%', background: C.rose, boxShadow: '0 0 6px rgba(26,48,96,.5)', marginLeft: 2, flexShrink: 0 }} />
          )}
        </button>
      );
    }
    ...
    return (
      <button
        ...
        style={{
          ...
          color: isActive ? C.crimson : 'rgba(26,48,96,0.5)',
        ...
        {tab.label}
        {isActive && (
          <div style={{ marginLeft: 'auto', width: 4, height: 4, borderRadius: '50%', background: C.rose, boxShadow: '0 0 8px rgba(26,48,96,0.5)', flexShrink: 0 }} />
        )}
      </button>
    );
  };
    ...
    <>
      ...
      <div className="rm-app-shell">
        ...
        <aside className="rm-sidebar" style={{
          ...
          <div style={{ position: 'absolute', top: 0, left: '15%', right: '15%', height: 1, background: `linear-gradient(90deg,transparent,rgba(26,48,96,0.4),transparent)` }} />
          ...
          <div style={{ textAlign: 'center', padding: `0 ${SP.xl}px ${SP.xxl}px`, borderBottom: `1px solid ${C.borderFaint}` }}>
            ...
            <span style={{ fontFamily: F.heading, fontSize: FS.micro, letterSpacing: '.22em', textTransform: 'uppercase' as const, color: 'rgba(26,48,96,0.35)' }}>
              Royal Edition
            </span>
          </div>
          ...
          <nav style={{ flex: 1, padding: `18px 14px`, display: 'flex', flexDirection: 'column', gap: SP.xs }}>
            <p style={{ fontFamily: F.heading, fontSize: FS.micro, letterSpacing: '.28em', textTransform: 'uppercase' as const, color: 'rgba(26,48,96,0.3)', padding: `0 10px`, marginBottom: SP.sm }}>
              Navigare
            </p>
          ...
          <div style={{ padding: `0 14px` }}>
            <div style={{
              ...
              background: isProfileComplete ? 'rgba(26,48,96,0.05)' : 'rgba(255,140,0,0.05)',
              border: `1px solid ${isProfileComplete ? 'rgba(26,48,96,0.2)' : 'rgba(255,140,0,0.22)'}`,
              borderRadius: BR.lg, position: 'relative', overflow: 'hidden',
            }}>
              <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,${isProfileComplete ? 'rgba(26,48,96,0.35)' : 'rgba(255,140,0,0.28)'},transparent)` }} />
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: isProfileComplete ? 7 : 0 }}>
                <PulseDot active={isProfileComplete} />
                <span style={{ fontFamily: F.heading, fontSize: FS.micro, letterSpacing: '.2em', textTransform: 'uppercase' as const, color: isProfileComplete ? 'rgba(26,48,96,0.8)' : 'rgba(255,165,0,0.8)' }}>
                  {isProfileComplete ? 'Link Activ' : 'Configurare Necesară'}
                </span>
              </div>
              {isProfileComplete && (
                <p style={{ fontFamily: F.body, fontSize: FS.base, fontStyle: 'italic', color: 'rgba(26,48,96,0.5)', wordBreak: 'break-all', lineHeight: 1.5 }}>
                  www.vibeinvite.ro/invitatie/royal/<strong style={{ color: 'rgba(26,48,96,0.8)', fontStyle: 'normal' }}>{weddingData.custom_slug}</strong>
                </p>
              )}
            </div>

            <div style={{ display: 'flex', alignItems: 'center', marginBottom: SP.sm }}>
              <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,transparent,rgba(26,48,96,0.2))` }} />
              <div style={{ width: 4, height: 4, background: 'rgba(26,48,96,0.35)', transform: 'rotate(45deg)', margin: `0 6px` }} />
              <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg,rgba(26,48,96,0.2),transparent)` }} />
            </div>
            ...
            <p style={{ fontFamily: F.heading, fontSize: FS.micro, letterSpacing: '.18em', textTransform: 'uppercase' as const, color: 'rgba(26,48,96,0.2)', textAlign: 'center', marginTop: 14 }}>
              VibeInvite © 2026
            </p>
          </div>

          <div style={{ position: 'absolute', bottom: 0, left: '15%', right: '15%', height: 1, background: `linear-gradient(90deg,transparent,rgba(26,48,96,0.25),transparent)` }} />
        </aside>
        ...
        <main className="rm-main">
          <div style={{
            ...
            <svg viewBox="0 0 160 160" fill="none" style={{ transform: 'scale(-1,1)', width: '100%', height: '100%' }}>
              <path d="M8 8 L8 120 M8 8 L120 8" stroke="rgba(26,48,96,0.6)" strokeWidth="1.2" />
              <path d="M18 18 L18 100 M18 18 L100 18" stroke="rgba(26,48,96,0.4)" strokeWidth=".7" />
              <ellipse cx="8" cy="8" rx="6" ry="6" fill="rgba(26,48,96,0.3)" />
            </svg>
          </div>
        ...
        <nav className="rm-tablet-nav">
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(26,48,96,0.12),transparent)' }} />
          {TABS.map(tab => <TabButton key={tab.id} tab={tab} variant="tablet" />)}
        </nav>

        {/* ── MOBILE BOTTOM NAV ───────────────────────── */}
        <nav className="rm-mobile-nav">
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: `linear-gradient(90deg,transparent,rgba(26,48,96,0.3),transparent)` }} />
