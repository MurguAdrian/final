...
import { C, F, FS, SP, BR, SH, GR, LY, KEYFRAMES, FONTS_IMPORT } from '../stelutaTokens';
...
const thStyle: React.CSSProperties = {
  ...
  textAlign: 'left',
  fontWeight: 700,
  borderBottom: `1px solid rgba(110,231,183,.18)`,
  whiteSpace: 'nowrap',
  position: 'sticky',
  top: 0,
...
const SUMMARY_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .ast-row:hover td { background: rgba(110,231,183,.05) !important; }
...
export function SummarySection() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `invitati_steluta_${data?.weddingDetails?.custom_slug || 'export'}.csv`);
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 14, fontFamily: F.ui, fontSize: FS.xs, letterSpacing: '.28em', textTransform: 'uppercase', color: C.textMuted }}>
      <div style={{ width: 18, height: 18, border: `1.5px solid rgba(110,231,183,.25)`, borderTopColor: C.accent, borderRadius: '50%', animation: 'ast-spin 1s linear infinite' }} />
      Se încarcă...
      ...
      <div style={{ animation: 'ast-fade-in .55s ease both', fontFamily: F.mono }}>
        ...
        <div className="ast-stats-grid">
          {[
            ...
            { icon: '👶',  label: 'Copii',          value: stats.copii  ?? 0,    color: '#D97706'     },
          ].map(s => (
        ...
        <div style={{ ...sCard, padding: 0, boxShadow: SH.card }}>
          <div style={{ padding: 'clamp(16px,3vw,24px)', borderBottom: `1px solid rgba(110,231,183,.16)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: SP.md }}>
            <div>
              ...
              <tbody>
                {guests.length > 0 ? (
                  guests.map((guest, i) => {
                    ...
                    return (
                      <tr key={guest.id ?? i} className="ast-row" style={{ borderBottom: `1px solid rgba(110,231,183,.10)`, transition: 'background .2s' }}>
                        <td style={tdStyle}>
                          <span style={{ fontFamily: F.mono, fontSize: 'clamp(13px,2vw,15px)', fontWeight: 600, color: C.text }}>{guestName}</span>
                        </td>
                        <td style={tdStyle}>
                          {guest.is_coming
                            ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: BR.pill, background: 'rgba(52,211,153,.12)', border: '1px solid rgba(52,211,153,.28)', fontFamily: F.ui, fontSize: FS.micro, letterSpacing: '.12em', fontWeight: 700, color: C.success, whiteSpace: 'nowrap' }}>✓ VINE</span>
                        ...
                        <td style={tdStyle} className="td-details">
                          ...
                          {!guest.partner_name && !guest.kids_count && (
                            <span style={{ color: 'rgba(110,231,183,.3)', fontSize: 14 }}>—</span>
                          )}
                        </td>
                      </tr>
                    );
