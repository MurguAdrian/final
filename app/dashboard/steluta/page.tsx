...
import { C, F, FS, SP, BR, LY, GR, SH, KEYFRAMES, FONTS_IMPORT } from './stelutaTokens';
...
const GCSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ${KEYFRAMES}
  ::selection { background: rgba(110,231,183,.28); color: ${C.text}; }
  .ast-input { transition: border-color .2s, box-shadow .2s; }
  .ast-input:focus { border-color: rgba(5,150,105,.55) !important; box-shadow: ${SH.glow}; outline: none; }
  .ast-ghost:hover { border-color: rgba(110,231,183,.4) !important; color: ${C.text} !important; }
  ...
  .ast-invite-banner {
    ...
    border-bottom: 1px solid rgba(110,231,183,.2);
  ...
  .ast-copy-btn:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(5,150,105,.45); }

  .ast-open-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(5,150,105,.08); color: ${C.accentDark};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
  ...
  .ast-open-btn:hover { background: rgba(5,150,105,.16); border-color: rgba(5,150,105,.42); }

  .ast-share-btn {
    display: inline-flex; align-items: center; justify-content: center;
    gap: ${SP.sm}px; padding: ${SP.xs + 2}px ${SP.lg - 2}px;
    border-radius: ${BR.pill}px;
    background: rgba(252,211,77,.10); color: ${C.textMuted};
    font-family: ${F.ui};
    font-size: ${FS.xs}px; font-weight: 700;
    letter-spacing: .12em; text-transform: uppercase;
  ...
  .ast-share-btn:hover { background: rgba(252,211,77,.2); border-color: rgba(252,211,77,.5); color: #92400E; }
...
export default function StelutaDashboard() {
  ...
  const inviteLink     = customSlug ? `https://www.vibeinvite.ro/invitatie/steluta/${customSlug}` : '';
        ...
        <header className="ast-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: SP.sm, minWidth: 0 }}>
            ...
            <span style={{
              ...
              <span style={{ color: C.textMuted, fontWeight: 400, fontSize: FS.tiny, marginLeft: SP.sm }}>stelu​ța</span>
            </span>
          </div>
