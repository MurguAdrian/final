...
import { C, F, FS, SP, BR, SH, GR, LY, KEYFRAMES, FONTS_IMPORT } from '../ursuletTokens';
...
const thStyle: React.CSSProperties = {
  ...
  textAlign: 'left',
  fontWeight: 700,
  borderBottom: `1px solid rgba(215,190,168,.28)`,
  whiteSpace: 'nowrap',
  position: 'sticky',
  top: 0,
...
const SUMMARY_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .ast-row:hover td { background: rgba(122,170,192,.04) !important; }
  .ast-export-btn:hover {
    background: rgba(89,176,227,.12) !important;
    border-color: rgba(89,176,227,.38) !important;
...
export function SummarySection() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `invitati_ursulet_${data?.weddingDetails?.custom_slug || 'export'}.csv`);
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 14, fontFamily: F.ui, fontSize: FS.xs, letterSpacing: '.28em', textTransform: 'uppercase', color: C.textMuted }}>
      <div style={{ width: 18, height: 18, border: `1.5px solid rgba(215,190,168,.35)`, borderTopColor: C.accent, borderRadius: '50%', animation: 'ast-spin 1s linear infinite' }} />
      Se încarcă...
      ...
      <div style={{ animation: 'ast-fade-in .55s ease both', fontFamily: F.mono }}>
        ...
        <div className="ast-stats-grid">
          {[
            ...
            { icon: '👶',  label: 'Copii',          value: stats.copii  ?? 0,    color: C.goldDim     },
          ].map(s => (
        ...
        <div style={{ ...sCard, padding: 0, boxShadow: SH.card }}>
          <div style={{ padding: 'clamp(16px,3vw,24px)', borderBottom: `1px solid rgba(215,190,168,.22)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: SP.md }}>
            <div>
              ...
              <tbody>
                {guests.length > 0 ? (
                  guests.map((guest, i) => {
                    ...
                    return (
                      <tr key={guest.id ?? i} className="ast-row" style={{ borderBottom: `1px solid rgba(215,190,168,.14)`, transition: 'background .2s' }}>
                        <td style={tdStyle}>
                        ...
                        <td style={tdStyle} className="td-details">
                          ...
                          {!guest.partner_name && !guest.kids_count && (
                            <span style={{ color: 'rgba(184,216,240,.4)', fontSize: 14 }}>—</span>
                          )}
                        </td>
                      </tr>
                    );
