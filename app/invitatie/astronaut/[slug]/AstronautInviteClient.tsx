...
const Comet = ({ style }: { style?: React.CSSProperties }) => (
  <svg style={{ display: 'block', ...style }} viewBox="0 0 60 24" fill="none">
    <path d="M58 2L4 12L58 22C40 18 30 16 30 12C30 8 40 6 58 2Z" fill="url(#cG)" opacity=".8" />
    <circle cx="6" cy="12" r="4" fill="#FFF8E0" />
    <defs><linearGradient id="cG" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#FFF8E0" stopOpacity="0" /><stop offset="1" stopColor="#FFF8E0" stopOpacity=".9" /></linearGradient></defs>
  </svg>
);
...
/* ── RSVP DEADLINE: 7 days before party ─────────────────────── */
...
const CSS_ANIM = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @keyframes astr-shimmer{0%{background-position:-350px 0}100%{background-position:350px 0}}
  @keyframes astr-pulse{0%,100%{opacity:.42}50%{opacity:.9}}
  @keyframes astr-fadeIn{from{opacity:0}to{opacity:1}}
  @keyframes astr-slideUp{from{opacity:0;transform:scale(.92) translateY(18px)}to{opacity:1;transform:scale(1) translateY(0)}}
  @media(max-width:480px){body{overflow:hidden}}
`;
  ...
  return (
    <>
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 clamp(16px,4vw,28px)', background: 'rgba(20,21,43,.9)', borderBottom: '1px solid rgba(156,182,232,.14)', backdropFilter: 'blur(14px)' }}>
        ...
        <a href="/preturi" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 100, background: 'rgba(124,107,196,.12)', border: '1px solid rgba(124,107,196,.25)', color: '#F4F6FB', fontFamily: "'Nunito',sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', textDecoration: 'none' }}>
          Alege Tema
        </a>
      </header>
      ...
      {phase !== 'invite' && (
        <div style={{ position: 'fixed', inset: 0, top: 56, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '16px 22px', width: '100%' }}>
            ...
            <div onClick={startLaunch} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && startLaunch()} style={{ position: 'relative', width: 'min(94vw,420px)', height: 'clamp(220px,46vw,340px)', cursor: 'pointer', marginTop: 6 }}>
              <div style={{ position: 'absolute', left: '50%', bottom: '8%', width: 'clamp(80px,22vw,130px)', transform: phase === 'launch' ? 'translate(-50%,-160vh) scale(.7)' : 'translate(-50%,0)', transition: 'transform 1.85s cubic-bezier(.5,.03,.5,1)', zIndex: 5 }}>
                <div style={{ animation: phase === 'launch' ? 'none' : 'astr-bob 2.6s ease-in-out infinite' }}>
                  <RocketSVG flame={phase === 'launch'} />
                </div>
              </div>
            ...
            <p style={{ fontFamily: "'Quicksand',sans-serif", fontSize: 'clamp(10px,1.2vw,12px)', letterSpacing: '.24em', textTransform: 'uppercase', color: '#9CB6E8', fontWeight: 600, animation: phase === 'launch' ? 'none' : 'astr-pulse 2.8s ease-in-out infinite 1.4s' }}>
              {phase === 'launch' ? '✦  Decolare spre invitație…' : 'Atinge pentru lansare'}
            </p>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, top: 56, overflowY: 'auto', overflowX: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: 720, margin: '0 auto', padding: 'clamp(32px,5vw,52px) clamp(18px,4vw,28px) 60px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
                ...
                {events.map((ev, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: 16, marginBottom: idx < events.length - 1 ? 22 : 0 }}>
                    ...
                    <div style={{ flex: 1, borderRadius: 16, border: '1px solid rgba(156,182,232,.16)', background: 'rgba(255,255,255,.04)', backdropFilter: 'blur(10px)', padding: 'clamp(12px,2vw,16px) clamp(14px,2.5vw,20px)', boxShadow: '0 6px 26px rgba(20,20,50,.3)' }}>
                      <span style={{ fontFamily: "'Quicksand',sans-serif", fontSize: 8.5, letterSpacing: '.2em', textTransform: 'uppercase', color: '#F4D87E', fontWeight: 600, display: 'block', marginBottom: 2 }}>{ev.type}</span>
                      <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(14px,1.8vw,17px)', fontStyle: 'italic', fontWeight: 400, color: '#F4F6FB', lineHeight: 1.2, marginBottom: 6 }}>{ev.name}</p>
                      <p style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 'clamp(11px,1.3vw,13px)', color: '#F4F6FB', marginBottom: 8 }}>{ev.venue}</p>
                      {ev.time && ev.dateDisplay && (
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(244,216,126,.1)', border: '1px solid rgba(244,216,126,.25)', borderRadius: 100, padding: '3px 10px', fontFamily: "'Quicksand',sans-serif", fontSize: 8.5, letterSpacing: '.1em', textTransform: 'uppercase', fontWeight: 600, color: '#F4D87E', marginBottom: 10 }}>
                          🚀 {ev.dateDisplay} · ora {ev.time}
                        </div>
                      )}
                      {(ev.waze || ev.maps) && (
            ...
            <div style={{ ...a(.68), textAlign: 'center', width: '100%', maxWidth: 380 }}>
              <p style={{ fontFamily: "'Cormorant',serif", fontSize: 'clamp(14px,1.7vw,17px)', fontStyle: 'italic', color: '#B8C4E8', marginBottom: 16, lineHeight: 1.6, letterSpacing: '.03em' }}>
                {rsvpDeadline ? (
                  <>Confirmați participarea până pe <strong style={{ color: '#F4D87E', fontStyle: 'normal' }}>{rsvpDeadline}</strong></>
                ) : 'Confirmați participarea'}
              </p>
              <button
      ...
      {showRsvp && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(10,10,28,.72)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, animation: 'astr-fadeIn .28s ease', overflowY: 'auto' }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ background: 'linear-gradient(165deg,#23254A,#1A1B3A)', borderRadius: 26, padding: 'clamp(24px,4vw,38px) clamp(18px,4vw,32px)', maxWidth: 460, width: '100%', border: '1px solid rgba(156,182,232,.22)', boxShadow: '0 40px 110px rgba(0,0,10,.6)', animation: 'astr-slideUp .32s cubic-bezier(.4,0,.2,1)', maxHeight: '92dvh', overflowY: 'auto', position: 'relative' }}
          >
            <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(124,107,196,.6),transparent)' }} />
            <AstronautRsvpForm orderId={orderId} onClose={() => setShowRsvp(false)} />
          </div>
        </div>
      )}
    </>
