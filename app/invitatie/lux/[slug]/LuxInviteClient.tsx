...
const MODAL_OVERLAY: React.CSSProperties = {
  ...
  padding: 16, animation: 'fadeIn .28s ease',
...
const GOLD_BTN: React.CSSProperties = {
  ...
  background: 'linear-gradient(135deg,#8B6914 0%,#D4AF37 45%,#F5D678 55%,#D4AF37 70%,#8B6914 100%)',
  color: '#0A0803', fontFamily: "'Cinzel',serif", fontSize: 11, fontWeight: 700,
  letterSpacing: '.2em', textTransform: 'uppercase' as const, border: 'none',
  cursor: 'pointer', boxShadow: '0 8px 28px rgba(212,175,55,.32)', transition: 'transform .2s,box-shadow .2s',
};

const NAV_BTN: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
  padding: '9px 14px', borderRadius: 8, fontFamily: "'Cinzel',serif", fontSize: 10,
  fontWeight: 600, letterSpacing: '.12em', cursor: 'pointer', flex: 1,
  whiteSpace: 'nowrap' as const, border: 'none', textDecoration: 'none',
};
...
const GLOBAL_CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  body{
    font-family:'Lato',sans-serif;background:#050401;color:#F5E6A8;
    height:100%;min-height:100dvh;overflow:hidden;overscroll-behavior:none;
...
function EnvelopeScreen({ onOpen, phase, brideName, groomName, initials, weddingDateDisplay }: {
  onOpen: () => void; phase: Phase; brideName: string; groomName: string; initials: string; weddingDateDisplay: string | null;
}) {
  return (
    <div style={{ position: 'fixed', inset: 0, top: 56, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', overscrollBehavior: 'none' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, padding: '20px 24px' }}>
        ...
        <div onClick={onOpen} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && onOpen()} style={{ animation: 'envFloat 5s ease-in-out infinite, fadeUp .9s ease both .2s', position: 'relative', width: 'clamp(290px,44vw,540px)', cursor: 'pointer', userSelect: 'none', filter: 'drop-shadow(0 40px 80px rgba(0,0,0,.7))' }}>
          <div style={{ position: 'absolute', bottom: -20, left: '8%', right: '8%', height: 24, background: 'radial-gradient(ellipse,rgba(212,175,55,.18) 0%,transparent 70%)', filter: 'blur(12px)', zIndex: 0 }} />
          <div style={{ position: 'absolute', left: '8%', right: '8%', bottom: '4%', height: '62%', zIndex: phase === 'opening' ? 30 : 2, background: 'linear-gradient(170deg,#1A1408 0%,#0D0A04 100%)', border: '1px solid rgba(212,175,55,.35)', borderRadius: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: phase === 'opening' ? '0 40px 100px rgba(0,0,0,.9),0 0 60px rgba(212,175,55,.2)' : '0 2px 8px rgba(0,0,0,.4)', transform: phase === 'opening' ? 'translateY(-145%) scale(1.06) rotate(-0.6deg)' : 'translateY(0)', transition: 'transform 1.4s cubic-bezier(.22,.1,.2,1) .2s,box-shadow 1.4s ease .2s', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0, opacity: 0.07, backgroundImage: 'repeating-linear-gradient(0deg,#D4AF37 0,#D4AF37 1px,transparent 1px,transparent 32px)' }} />
            <div style={{ position: 'absolute', top: 8, left: 8, right: 8, bottom: 8, border: '1px solid rgba(212,175,55,.2)', borderRadius: 2 }} />
...
function InviteScreen({ props }: { props: LuxInviteClientProps }) {
  ...
  // Rezolvam afisarea parintilor: preferam campurile separate, fallback la parentsNames
  ...
  // Fallback: daca nu avem campurile separate, split parentsNames la " si "
  let fallbackBride = '';
  let fallbackGroom = '';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, top: 56, overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 700, margin: '0 auto', padding: 'clamp(36px,6vw,64px) clamp(16px,4vw,32px) 60px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
        ...
        {/* ── PARINTI: 2 randuri separate ── */}
        {hasAnyParents && (
          <div style={{ ...a(0.3), textAlign: 'center', marginTop: 14, marginBottom: 4 }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(12px,1.4vw,14px)', fontStyle: 'italic', color: 'rgba(212,175,55,.5)', letterSpacing: '.06em', marginBottom: 6 }}>Împreună cu părinții</p>
            {displayParentsBride && (
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(14px,1.6vw,16px)', fontStyle: 'italic', fontWeight: 300, color: 'rgba(245,230,168,.65)', letterSpacing: '.04em', marginBottom: displayParentsGroom ? 4 : 0 }}>
                {displayParentsBride}
              </p>
            )}
            {displayParentsGroom && (
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(14px,1.6vw,16px)', fontStyle: 'italic', fontWeight: 300, color: 'rgba(245,230,168,.65)', letterSpacing: '.04em', marginBottom: 0 }}>
                {displayParentsGroom}
              </p>
            )}
          </div>
