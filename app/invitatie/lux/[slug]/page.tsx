...
export async function generateMetadata(
  ...
  const sql = neon(process.env.DATABASE_URL!);
  const data = await sql`SELECT bride_name, groom_name, wedding_date, location_name FROM wedding_settings WHERE custom_slug = ${params.slug} LIMIT 1`;
...
/* ════════════════════════════════════════════════════════════════
   SERVER COMPONENT — fetch date DB
════════════════════════════════════════════════════════════════ */
export default async function InvitationPage({ params }: { params: { slug: string } }) {
  const sql = neon(process.env.DATABASE_URL!);
  ...
  const isGalleryActive =
    ...
    s.photos_expires_at &&
    new Date(s.photos_expires_at).getTime() > new Date().getTime();
...
/* ════════════════════════════════════════════════════════════════
   CLIENT COMPONENT — toată logica UI / animații / modale
════════════════════════════════════════════════════════════════ */
'use client';
        ...
        <div onClick={onOpen} role="button" tabIndex={0} onKeyDown={e=>e.key==='Enter'&&onOpen()}
          ...
          <div style={{position:'absolute',left:'8%',right:'8%',bottom:'4%',height:'62%',zIndex:phase==='opening'?30:2,background:'linear-gradient(170deg,#1A1408 0%,#0D0A04 100%)',border:'1px solid rgba(212,175,55,.35)',borderRadius:4,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:10,boxShadow:phase==='opening'?'0 40px 100px rgba(0,0,0,.9),0 0 60px rgba(212,175,55,.2)':'0 2px 8px rgba(0,0,0,.4)',transform:phase==='opening'?'translateY(-145%) scale(1.06) rotate(-0.6deg)':'translateY(0)',transition:'transform 1.4s cubic-bezier(.22,.1,.2,1) .2s,box-shadow 1.4s ease .2s',overflow:'hidden'}}>
            ...
            <div style={{textAlign:'center',padding:'0 20px',position:'relative',zIndex:1}}>
              ...
              {weddingDateDisplay && (
                <p style={{fontFamily:"'Cinzel',serif",fontSize:'clamp(7px,.9vw,9px)',letterSpacing:'.3em',textTransform:'uppercase',color:'rgba(212,175,55,.7)',fontWeight:400}}>{weddingDateDisplay}</p>
              )}
            </div>
          </div>
          {/* Envelope body */}
...
/* ════════════════════════════════════════════════════════════════
   INVITE SCREEN
════════════════════════════════════════════════════════════════ */
function InviteScreen({ props }: { props: Props }) {
  const {
    slug, brideName, groomName, nasiNames, parentsNames,
    weddingDateISO, weddingDateDisplay, weddingTime, locationName,
    ...
    contactPhoneBride, contactPhoneGroom, orderId,
  } = props;
  ...
  const btn: React.CSSProperties = {
    ...
    fontWeight:600, letterSpacing:'.12em', cursor:'pointer', flex:1, whiteSpace:'nowrap',
    border:'none', textDecoration:'none',
  };

  const inputStyle: React.CSSProperties = {
    width:'100%', padding:'10px 14px', borderRadius:8,
    border:'1px solid rgba(212,175,55,.25)', background:'rgba(212,175,55,.05)',
    fontFamily:"'Cormorant Garamond',serif", fontSize:14, color:'#F5E6A8', outline:'none',
  };

  const radioLabel: React.CSSProperties = {
    flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:8,
    padding:'10px 10px', borderRadius:8, border:'1px solid rgba(212,175,55,.2)',
    background:'rgba(212,175,55,.04)', cursor:'pointer', fontFamily:"'Cinzel',serif",
    fontSize:11, letterSpacing:'.08em', color:'rgba(212,175,55,.8)',
  ...
  const modalOverlay: React.CSSProperties = {
    ...
    justifyContent:'center', padding:16, animation:'fadeIn .28s ease', overflowY:'auto',
  };

  const modalBox: React.CSSProperties = {
    background:'linear-gradient(170deg,#1A1408,#0A0803)', borderRadius:20,
    padding:'clamp(24px,4vw,38px) clamp(18px,4vw,32px)', maxWidth:480, width:'100%',
    border:'1px solid rgba(212,175,55,.25)',
    boxShadow:'0 40px 100px rgba(0,0,0,.9),0 0 60px rgba(212,175,55,.12)',
    animation:'slideUp .32s cubic-bezier(.4,0,.2,1)', maxHeight:'90vh',
  ...
  const goldBtn: React.CSSProperties = {
    ...
    boxShadow:'0 8px 28px rgba(212,175,55,.32)', transition:'transform .2s,box-shadow .2s',
  };
    ...
    <div style={{position:'fixed',inset:0,top:56,overflowY:'auto',overflowX:'hidden'}}>
      ...
      <div style={{position:'relative',zIndex:10,maxWidth:700,margin:'0 auto',padding:'clamp(36px,6vw,64px) clamp(16px,4vw,32px) 60px',display:'flex',flexDirection:'column',alignItems:'center',gap:0}}>
        ...
        <div style={{...a(.1),textAlign:'center',marginBottom:6}}>
          <span style={{display:'block',fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(58px,10vw,112px)',fontWeight:600,fontStyle:'italic',color:'#F5E6A8',lineHeight:.92,letterSpacing:'-.01em',textShadow:'0 0 60px rgba(212,175,55,.25)'}}>{brideName}</span>
          ...
          <span style={{display:'block',fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(58px,10vw,112px)',fontWeight:600,fontStyle:'italic',color:'#F5E6A8',lineHeight:.92,letterSpacing:'-.01em',textShadow:'0 0 60px rgba(212,175,55,.25)'}}>{groomName}</span>
        </div>
        ...
        {(weddingDateDisplay || locationName) && (
          <div style={{...a(.22),textAlign:'center',marginBottom:20}}>
            {weddingDateDisplay && (
              <p style={{fontFamily:"'Cinzel',serif",fontSize:'clamp(14px,1.8vw,18px)',letterSpacing:'.22em',color:'#D4AF37',fontWeight:400,marginBottom:6,textTransform:'capitalize'}}>{weddingDateDisplay}</p>
            )}
            {locationName && (
              <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(14px,1.7vw,18px)',fontStyle:'italic',fontWeight:300,color:'rgba(245,230,168,.55)',letterSpacing:'.06em'}}>{locationName}</p>
            )}
          </div>
        )}
        ...
        <div style={{...a(.46),width:'100%',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,260px),1fr))',gap:'clamp(12px,2vw,20px)',maxWidth:640}}>
          {/* Petrecere card */}
          <div style={{borderRadius:16,overflow:'hidden',border:'1px solid rgba(212,175,55,.2)',background:'rgba(255,255,255,.03)',backdropFilter:'blur(12px)',boxShadow:'0 8px 40px rgba(0,0,0,.5)',transition:'transform .25s ease,box-shadow .25s ease'}}
            ...
            <div style={{padding:'14px 18px 16px'}}>
              ...
              {weddingDateDisplay && weddingTime && (
                <div style={{display:'inline-flex',alignItems:'center',gap:5,background:'rgba(212,175,55,.08)',border:'1px solid rgba(212,175,55,.2)',borderRadius:100,padding:'4px 12px',fontFamily:"'Cinzel',serif",fontSize:8,letterSpacing:'.14em',textTransform:'uppercase',color:'rgba(212,175,55,.7)',marginBottom:12,marginTop:4}}>
                ...
                <div style={{display:'flex',gap:8,marginTop:8}}>
                  {wazeUrl && (
                    <a href={wazeUrl} target="_blank" rel="noopener noreferrer"
                      style={{...btn,background:'linear-gradient(135deg,rgba(8,162,212,.25),rgba(8,162,212,.15))',border:'1px solid rgba(8,162,212,.3)',color:'rgba(140,210,240,.9)'}} >
                      <WazeIcon/> Waze
                    </a>
                  )}
                  {googleMapsUrl && (
                    <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer"
                      style={{...btn,background:'linear-gradient(135deg,rgba(76,175,79,.22),rgba(76,175,79,.12))',border:'1px solid rgba(76,175,79,.28)',color:'rgba(120,210,120,.9)'}}>
                      <MapsIcon/> Maps
                    </a>
                  )}
                </div>
          ...
          {isReligiousActive && (
            <div style={{borderRadius:16,overflow:'hidden',border:'1px solid rgba(212,175,55,.2)',background:'rgba(255,255,255,.03)',backdropFilter:'blur(12px)',boxShadow:'0 8px 40px rgba(0,0,0,.5)',transition:'transform .25s ease,box-shadow .25s ease'}}
              onMouseEnter={e=>{(e.currentTarget as HTMLDivElement).style.transform='translateY(-4px)';(e.currentTarget as HTMLDivElement).style.boxShadow='0 20px 60px rgba(0,0,0,.7)'}}
              onMouseLeave={e=>{(e.currentTarget as HTMLDivElement).style.transform='';(e.currentTarget as HTMLDivElement).style.boxShadow='0 8px 40px rgba(0,0,0,.5)'}}>
              <div style={{padding:'16px 18px 12px',background:'linear-gradient(135deg,rgba(212,175,55,.15) 0%,rgba(212,175,55,.06) 100%)',borderBottom:'1px solid rgba(212,175,55,.15)',display:'flex',alignItems:'center',gap:12}}>
                <div style={{width:40,height:40,borderRadius:10,background:'rgba(212,175,55,.12)',border:'1px solid rgba(212,175,55,.3)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="#D4AF37" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{width:20,height:20}}><path d="M12 2L12 6M10 4h4"/><rect x="4" y="9" width="16" height="12" rx="1"/><path d="M9 21V14a3 3 0 0 1 6 0v7"/><path d="M4 9l8-4 8 4"/></svg>
                </div>
                <div>
              ...
              <div style={{padding:'14px 18px 16px'}}>
                ...
                {religiousWaze && (
                  <a href={religiousWaze} target="_blank" rel="noopener noreferrer"
                    style={{...btn,background:'linear-gradient(135deg,rgba(8,162,212,.25),rgba(8,162,212,.15))',border:'1px solid rgba(8,162,212,.3)',color:'rgba(140,210,240,.9)',flex:'none'}}>
        ...
        {ourStory && (
          <>
            <div style={{...a(.50),display:'flex',alignItems:'center',gap:0,width:'100%',maxWidth:440,margin:'24px auto'}}><GoldDivider/></div>
            <div style={{...a(.52),width:'100%',maxWidth:560,textAlign:'center',padding:'10px 0'}}>
              ...
              <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(16px,2vw,20px)',fontStyle:'italic',fontWeight:300,color:'rgba(245,230,168,.75)',lineHeight:1.85,letterSpacing:'.02em'}}>
        ...
        {(isAccommodationActive || isTransportActive) && (
          <div style={{...a(.54),display:'flex',gap:16,justifyContent:'center',padding:'16px 0',marginTop:8}}>
            {isAccommodationActive && (
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:6,padding:'16px 20px',background:'rgba(212,175,55,.05)',border:'1px solid rgba(212,175,55,.18)',borderRadius:14}}>
                <span style={{fontSize:'2rem'}}>🏠</span>
                <span style={{fontFamily:"'Cinzel',serif",fontSize:8,letterSpacing:'.16em',textTransform:'uppercase',color:'rgba(212,175,55,.55)'}}>Cazare</span>
              </div>
            )}
            {isTransportActive && (
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:6,padding:'16px 20px',background:'rgba(212,175,55,.05)',border:'1px solid rgba(212,175,55,.18)',borderRadius:14}}>
                <span style={{fontSize:'2rem'}}>🚌</span>
                <span style={{fontFamily:"'Cinzel',serif",fontSize:8,letterSpacing:'.16em',textTransform:'uppercase',color:'rgba(212,175,55,.55)'}}>Transport</span>
              </div>
            )}
          </div>
        )}
        ...
        {(contactPhoneBride || contactPhoneGroom) && (
          <div style={{...a(.55),width:'100%',maxWidth:640,background:'rgba(212,175,55,.04)',border:'1px solid rgba(212,175,55,.18)',borderRadius:16,padding:'16px 20px',backdropFilter:'blur(8px)',boxShadow:'0 6px 30px rgba(0,0,0,.4)',marginTop:'clamp(12px,2vw,20px)'}}>
              ...
              {contactPhoneBride && (
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    <a href={`tel:${contactPhoneBride}`} style={{...btn,padding:'10px 18px',borderRadius:100,background:'rgba(212,175,55,.12)',border:'1px solid rgba(212,175,55,.3)',color:'#D4AF37'}}><PhoneIcon/> Telefon</a>
                    <a href={`https://wa.me/${contactPhoneBride.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" style={{...btn,padding:'10px 18px',borderRadius:100,background:'rgba(37,211,102,.12)',border:'1px solid rgba(37,211,102,.3)',color:'rgba(100,220,130,.9)'}}><WaIcon/> WhatsApp</a>
                  </div>
                </div>
              )}
              {contactPhoneGroom && (
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    <a href={`tel:${contactPhoneGroom}`} style={{...btn,padding:'10px 18px',borderRadius:100,background:'rgba(212,175,55,.12)',border:'1px solid rgba(212,175,55,.3)',color:'#D4AF37'}}><PhoneIcon/> Telefon</a>
                    <a href={`https://wa.me/${contactPhoneGroom.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" style={{...btn,padding:'10px 18px',borderRadius:100,background:'rgba(37,211,102,.12)',border:'1px solid rgba(37,211,102,.3)',color:'rgba(100,220,130,.9)'}}><WaIcon/> WhatsApp</a>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{...a(.58),display:'flex',alignItems:'center',gap:0,width:'100%',maxWidth:440,margin:'24px auto'}}><GoldDivider/></div>

        {/* MENIU BUTON */}
        {isMenuActive && menuDetails?.categories && (
          <div style={{...a(.59),width:'100%',maxWidth:400,marginBottom:16}}>
            <button onClick={()=>setMenuModal(true)}
              style={{...goldBtn,display:'flex',alignItems:'center',justifyContent:'center',gap:10,width:'100%',padding:'14px 0'}}
        ...
        {/* GALERIE BUTON */}
        {isGalleryActive && (
          <div style={{...a(.60),width:'100%',maxWidth:640,background:'linear-gradient(160deg,rgba(212,175,55,.07) 0%,rgba(212,175,55,.03) 100%)',border:'1.5px dashed rgba(212,175,55,.3)',borderRadius:20,padding:'clamp(22px,3vw,32px) clamp(18px,3vw,28px)',textAlign:'center',marginBottom:16}}>
            ...
            <button onClick={()=>setGalleryModal(true)}
              style={{display:'inline-flex',alignItems:'center',gap:10,padding:'13px 32px',borderRadius:4,background:'linear-gradient(135deg,#8B6914 0%,#D4AF37 45%,#F5D678 55%,#D4AF37 70%,#8B6914 100%)',color:'#0A0803',border:'none',cursor:'pointer',fontFamily:"'Cinzel',serif",fontSize:'clamp(10px,1.2vw,12px)',fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',boxShadow:'0 8px 28px rgba(212,175,55,.3)',transition:'transform .2s,box-shadow .2s'}}
              onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.transform='translateY(-2px)';(e.currentTarget as HTMLButtonElement).style.boxShadow='0 14px 40px rgba(212,175,55,.5)'}}
        ...
        <div style={{...a(.65),textAlign:'center',width:'100%',maxWidth:400}}>
          <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(14px,1.7vw,17px)',fontStyle:'italic',color:'rgba(212,175,55,.55)',marginBottom:16,lineHeight:1.7,letterSpacing:'.04em'}}>
            Vă rugăm să confirmați prezența Dvs.<br/>până la data evenimentului.
          </p>
          <button onClick={()=>setRsvpModal(true)}
            style={{...goldBtn,display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}
      ...
      {/* ══ MENIU MODAL ══ */}
      {menuModal && (
        <div onClick={()=>setMenuModal(false)} style={modalOverlay}>
              ...
              .map((cat: any, cIdx: number) => (
                <div key={cIdx} style={{marginBottom:32}}>
                  <div style={{display:'inline-block',borderBottom:'1px solid rgba(212,175,55,.3)',paddingBottom:10,marginBottom:16}}>
                    <h4 style={{color:'#fff',fontSize:'clamp(13px,1.6vw,16px)',margin:0,fontWeight:300,letterSpacing:'2px',fontFamily:"'Cinzel',serif"}}>
                      <span style={{marginRight:10}}>{cat.emoji}</span>{cat.label}
                    </h4>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:14}}>
                    {cat.items.map((item: any, iIdx: number) => (
                      <div key={iIdx}>
                        ...
                        {item.description && (
                          <div style={{fontSize:'clamp(11px,1.2vw,13px)',opacity:.55,fontStyle:'italic',marginTop:4,fontFamily:"'Cormorant Garamond',serif",color:'#F5E6A8'}}>{item.description}</div>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {/* ══ GALERIE MODAL ══ */}
      {galleryModal && (
        <div onClick={()=>setGalleryModal(false)} style={modalOverlay}>
          <div onClick={e=>e.stopPropagation()} style={{...modalBox,maxWidth:400,textAlign:'center'}}>
            <div style={{position:'absolute',top:0,left:'10%',right:'10%',height:1,background:'linear-gradient(90deg,transparent,rgba(212,175,55,.5),transparent)'}}/>
            ...
            <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontStyle:'italic',color:'rgba(212,175,55,.6)',marginBottom:24,lineHeight:1.8}}>
              Împărtășește momentele surprinse de tine cu mirii.<br/>Apasă butonul de mai jos pentru a încărca pozele tale.
            </p>
            <a href={`/invitatie/lux/${slug}/upload`}
              style={{display:'inline-flex',alignItems:'center',gap:10,padding:'13px 32px',borderRadius:4,background:'linear-gradient(135deg,#8B6914 0%,#D4AF37 45%,#F5D678 55%,#D4AF37 70%,#8B6914 100%)',color:'#0A0803',textDecoration:'none',fontFamily:"'Cinzel',serif",fontSize:11,fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',boxShadow:'0 8px 28px rgba(212,175,55,.3)',marginBottom:16}}>
              📷 Încarcă Pozele
            </a>
            <br/>
      ...
      {/* ══ RSVP MODAL ══ */}
      {rsvpModal && (
        <div onClick={()=>setRsvpModal(false)} style={modalOverlay}>
...
/* ════════════════════════════════════════════════════════════════
   CLIENT WRAPPER — gestionează plica / faza
════════════════════════════════════════════════════════════════ */
'use client';

function LuxInviteClient(props: Props) {
  ...
  return (
    <>
      ...
      <header style={{position:'fixed',top:0,left:0,right:0,zIndex:200,height:56,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 clamp(14px,4vw,28px)',background:'rgba(5,4,1,.95)',borderBottom:'1px solid rgba(212,175,55,.15)',backdropFilter:'blur(16px)'}}>
        <a href="/" style={{fontFamily:"'Cinzel',serif",fontSize:12,fontWeight:600,letterSpacing:'.24em',textTransform:'uppercase',color:'#D4AF37',textDecoration:'none',transition:'color .2s'}}
