...
export interface MinimalInviteClientProps {
  slug: string;
  brideName: string;
  groomName: string;
  initials: string;
...
const MODAL_OVERLAY: React.CSSProperties = {
  ...
  padding: 16, animation: 'mn-fadeIn .22s ease',
...
const GLOBAL_CSS = `
  @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;1,300;1,400;1,500&family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600&display=swap');
...
function EnvelopeScreen({ onOpen, phase, brideName, groomName, initials, weddingDateDisplay }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, top: 56, background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', overscrollBehavior: 'none' }}>
      ...
      <div aria-hidden style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', overflow: 'hidden' }}>
        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(140px,28vw,340px)', fontWeight: 700, fontStyle: 'italic', color: 'rgba(0,0,0,.03)', letterSpacing: '-.04em', userSelect: 'none', lineHeight: 1, whiteSpace: 'nowrap' }}>{brideName}</span>
      </div>

      {/* Left accent stripe */}
      <div style={{ position: 'absolute', top: 0, left: 0, width: 'clamp(4px,.5vw,6px)', height: '100%', background: ACCENT, zIndex: 1 }}/>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, padding: '24px', maxWidth: 560, width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, animation: 'mn-fadeUp .5s ease both' }}>
          ...
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10, letterSpacing: '.36em', textTransform: 'uppercase', color: LIGHT, fontWeight: 500 }}>Invitație · {weddingDateDisplay ? new Date(weddingDateDisplay).getFullYear() || new Date().getFullYear() + 1 : new Date().getFullYear() + 1}</p>
          <div style={{ width: 28, height: 2, background: ACCENT }}/>
        </div>

        <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(36px,6vw,68px)', fontWeight: 400, fontStyle: 'italic', color: DARK, textAlign: 'center', lineHeight: 1.08, animation: 'mn-fadeUp .65s ease both .06s', margin: 0, letterSpacing: '-.01em' }}>
          {brideName} &amp; {groomName}
        </h1>
        ...
        <div onClick={onOpen} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && onOpen()}
          ...
          <div style={{ position: 'absolute', bottom: -14, left: '10%', right: '10%', height: 18, background: 'radial-gradient(ellipse,rgba(0,0,0,.08) 0%,transparent 70%)', filter: 'blur(10px)', zIndex: 0 }}/>
          ...
          <div style={{
            ...
            boxShadow: phase === 'opening' ? '0 28px 70px rgba(0,0,0,.15),0 4px 20px rgba(0,0,0,.07)' : '0 2px 8px rgba(0,0,0,.04)',
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(7px,.8vw,9px)', letterSpacing: '.28em', textTransform: 'uppercase', color: ACCENT, fontWeight: 600, marginBottom: 4 }}>Nuntă · {weddingDateDisplay || ''}</p>
              <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(15px,2.5vw,22px)', fontStyle: 'italic', fontWeight: 400, color: DARK, lineHeight: 1.2 }}>{brideName} &amp; {groomName}</p>
            </div>
          </div>

          {/* Envelope body */}
          <div style={{ width: '100%', paddingTop: '60%', position: 'relative', zIndex: 5 }}>
            <div style={{ position: 'absolute', inset: 0, background: '#E8E4DE', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,.07)' }}>
              ...
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: ACCENT, opacity: .5 }}/>
            </div>
            {/* Wax seal */}
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-52%)', width: 'clamp(52px,8.5vw,76px)', height: 'clamp(52px,8.5vw,76px)', background: DARK, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 0 3px ${BG}, 0 0 0 5px ${ACCENT}, 0 8px 24px rgba(0,0,0,.15)`, zIndex: 10, opacity: phase === 'opening' ? 0 : 1, transition: 'opacity .2s' }}>
              <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(11px,1.8vw,17px)', fontStyle: 'italic', color: '#fff', letterSpacing: '.02em' }}>{initials}</span>
            </div>
            {/* Flap */}
...
function InviteScreen({ props }: { props: MinimalInviteClientProps }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, top: 56, overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain', background: BG }}>
      {/* Left accent */}
      <div style={{ position: 'fixed', top: 0, left: 0, width: 'clamp(4px,.5vw,6px)', height: '100%', background: ACCENT, zIndex: 10, pointerEvents: 'none' }}/>
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 720, margin: '0 auto', padding: 'clamp(44px,7vw,72px) clamp(24px,6vw,56px) 60px clamp(28px,6vw,60px)', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 0 }}>
        ...
        <div style={{ ...a(0), width: '100%', marginBottom: 48 }}>
          ...
          <div style={{ position: 'relative' }}>
            <span style={{ display: 'block', fontFamily: "'Playfair Display',serif", fontSize: 'clamp(52px,10vw,112px)', fontWeight: 400, fontStyle: 'italic', color: DARK, lineHeight: .88, letterSpacing: '-.02em' }}>{brideName}</span>
            ...
            <span style={{ display: 'block', fontFamily: "'Playfair Display',serif", fontSize: 'clamp(52px,10vw,112px)', fontWeight: 400, fontStyle: 'italic', color: DARK, lineHeight: .88, letterSpacing: '-.02em' }}>{groomName}</span>
            <span aria-hidden style={{ position: 'absolute', right: -8, top: '50%', transform: 'translateY(-50%)', fontFamily: "'Playfair Display',serif", fontSize: 'clamp(80px,18vw,200px)', fontWeight: 700, color: 'rgba(200,80,58,.05)', lineHeight: 1, userSelect: 'none', pointerEvents: 'none', letterSpacing: '-.04em' }}>27</span>
          </div>
        ...
        {nasiNames && (
          <div style={{ ...a(.14), width: '100%', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 'clamp(16px,3vw,32px)', alignItems: 'center', marginBottom: 40, paddingBottom: 32, borderBottom: `1px solid ${RULE}` }}>
            ...
            <div>
              ...
              <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(18px,2.6vw,26px)', fontStyle: 'italic', fontWeight: 400, color: DARK }}>{nasiNames}</p>
            </div>
          </div>
        )}
        ...
        {/* Părinți */}
        {parentsNames && (
          <div style={{ ...a(.17), width: '100%', marginBottom: 40, paddingBottom: 32, borderBottom: `1px solid ${RULE}` }}>
            ...
            <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(16px,2vw,20px)', fontStyle: 'italic', fontWeight: 400, color: MID }}>{parentsNames}</p>
          </div>
        )}
        ...
        {weddingDate && (
          <div style={{ ...a(.20), width: '100%', marginBottom: 40 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 'clamp(4px,1vw,8px)' }}>
              {[{ n: pad(cd.d), l: 'Zile', i: 0 }, { n: pad(cd.h), l: 'Ore', i: 1 }, { n: pad(cd.m), l: 'Minute', i: 2 }, { n: pad(cd.s), l: 'Secunde', flip: flipS, i: 3 }].map((u) => (
                <div key={u.l} style={{ background: u.i === 0 ? DARK : u.i === 1 ? ACCENT : u.i === 2 ? '#fff' : BG, padding: 'clamp(14px,2vw,22px) clamp(8px,1.5vw,16px)', border: `1px solid ${u.i >= 2 ? RULE : 'transparent'}` }}>
                  <span style={{ display: 'block', fontFamily: "'Playfair Display',serif", fontSize: 'clamp(32px,5.5vw,56px)', fontWeight: 400, fontStyle: 'italic', lineHeight: 1, color: u.i < 2 ? '#fff' : DARK, transition: 'opacity .12s', opacity: (u as any).flip ? .5 : 1 }}>{u.n}</span>
                  <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(7px,.78vw,9px)', letterSpacing: '.22em', textTransform: 'uppercase', color: u.i < 2 ? 'rgba(255,255,255,.6)' : LIGHT, display: 'block', marginTop: 4, fontWeight: 400 }}>{u.l}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,280px),1fr))', gap: 'clamp(10px,2vw,16px)' }}>
            ...
            <div style={{ background: DARK, padding: 'clamp(20px,3vw,28px)', position: 'relative', overflow: 'hidden', transition: 'transform .22s ease,box-shadow .22s ease' }}
              ...
              <div style={{ paddingLeft: 12 }}>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: '.3em', textTransform: 'uppercase', color: ACCENT, fontWeight: 600, marginBottom: 8 }}>Recepție</p>
                <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(17px,2.2vw,22px)', fontStyle: 'italic', fontWeight: 400, color: '#fff', marginBottom: 4, lineHeight: 1.25 }}>{locationName}</p>
                {weddingDateDisplay && <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: 'rgba(255,255,255,.4)', marginBottom: 12, letterSpacing: '.03em' }}>{weddingDateDisplay}</p>}
                {weddingTime && (
                  <div style={{ display: 'inline-block', background: ACCENT, padding: '4px 12px', marginBottom: 16 }}>
                    <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: '.18em', textTransform: 'uppercase', color: '#fff', fontWeight: 600 }}>ora {weddingTime}</span>
                  </div>
                )}
                {(wazeUrl || googleMapsUrl) && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    {wazeUrl && <a href={wazeUrl} target="_blank" rel="noopener noreferrer" style={{ ...NAV_BTN, border: '1px solid rgba(255,255,255,.18)', color: 'rgba(255,255,255,.65)', flex: 1, justifyContent: 'center', transition: 'all .18s' }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(255,255,255,.4)'; (e.currentTarget as HTMLAnchorElement).style.color = '#fff'; }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(255,255,255,.18)'; (e.currentTarget as HTMLAnchorElement).style.color = 'rgba(255,255,255,.65)'; }}><WazeIcon/> Waze</a>}
                    {googleMapsUrl && <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer" style={{ ...NAV_BTN, border: '1px solid rgba(255,255,255,.18)', color: 'rgba(255,255,255,.65)', flex: 1, justifyContent: 'center', transition: 'all .18s' }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(255,255,255,.4)'; (e.currentTarget as HTMLAnchorElement).style.color = '#fff'; }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(255,255,255,.18)'; (e.currentTarget as HTMLAnchorElement).style.color = 'rgba(255,255,255,.65)'; }}><MapsIcon/> Maps</a>}
                  </div>
                )}
              </div>
            ...
            {isReligiousActive && (
              <div style={{ background: ACCENT, padding: 'clamp(20px,3vw,28px)', position: 'relative', overflow: 'hidden', transition: 'transform .22s ease,box-shadow .22s ease' }}
                ...
                <div style={{ paddingLeft: 12 }}>
                  ...
                  <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(17px,2.2vw,22px)', fontStyle: 'italic', fontWeight: 400, color: '#fff', marginBottom: 4, lineHeight: 1.25 }}>{religiousLocation}</p>
                  {religiousDateDisplay && <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: 'rgba(255,255,255,.5)', marginBottom: 12, letterSpacing: '.03em' }}>{religiousDateDisplay}</p>}
                  {religiousTime && (
                    <div style={{ display: 'inline-block', background: 'rgba(0,0,0,.2)', padding: '4px 12px', marginBottom: 16 }}>
                      <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: '.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,.9)', fontWeight: 600 }}>ora {religiousTime}</span>
                    </div>
                  )}
                  {religiousWaze && (
                    <a href={religiousWaze} target="_blank" rel="noopener noreferrer" style={{ ...NAV_BTN, border: '1px solid rgba(255,255,255,.35)', color: 'rgba(255,255,255,.8)', flex: 'none', transition: 'all .18s' }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(255,255,255,.15)'; }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'transparent'; }}>
        ...
        {/* Our Story */}
        {ourStory && (
          <div style={{ ...a(.34), width: '100%', marginBottom: 40, padding: 'clamp(20px,3vw,28px)', background: '#fff', borderLeft: `4px solid ${RULE}`, boxShadow: '0 2px 16px rgba(0,0,0,.04)' }}>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: '.3em', textTransform: 'uppercase', color: ACCENT, fontWeight: 600, marginBottom: 10 }}>Povestea noastră</p>
            <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(15px,1.8vw,18px)', fontStyle: 'italic', fontWeight: 400, color: MID, lineHeight: 1.85 }}>&ldquo;{ourStory}&rdquo;</p>
          </div>
        )}

        {/* Contact */}
        {(contactPhoneBride || contactPhoneGroom) && (
          <div style={{ ...a(.36), width: '100%', marginBottom: 40, background: '#fff', padding: 'clamp(18px,2.5vw,24px) clamp(20px,3vw,28px)', borderLeft: `4px solid ${DARK}`, boxShadow: '0 2px 16px rgba(0,0,0,.04)', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {contactPhoneBride && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div>
                  ...
                  <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(17px,2.2vw,22px)', fontStyle: 'italic', color: DARK, marginBottom: 2 }}>{brideName}</p>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: MID, letterSpacing: '.06em', fontWeight: 400 }}>{contactPhoneBride}</p>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <a href={`tel:${contactPhoneBride}`} style={{ ...NAV_BTN, background: DARK, color: '#fff', padding: '10px 16px', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', transition: 'opacity .18s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '.75'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '1'}><PhoneIcon/> Telefon</a>
                  <a href={`https://wa.me/${contactPhoneBride.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ ...NAV_BTN, background: '#25D366', color: '#fff', padding: '10px 16px', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', transition: 'opacity .18s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '.82'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '1'}><WaIcon/> WhatsApp</a>
                </div>
              </div>
            )}
            {contactPhoneGroom && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: '.3em', textTransform: 'uppercase', color: ACCENT, fontWeight: 600, marginBottom: 3 }}>Contact Mire</p>
                  <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(17px,2.2vw,22px)', fontStyle: 'italic', color: DARK, marginBottom: 2 }}>{groomName}</p>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: MID, letterSpacing: '.06em', fontWeight: 400 }}>{contactPhoneGroom}</p>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <a href={`tel:${contactPhoneGroom}`} style={{ ...NAV_BTN, background: DARK, color: '#fff', padding: '10px 16px', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', transition: 'opacity .18s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '.75'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '1'}><PhoneIcon/> Telefon</a>
                  <a href={`https://wa.me/${contactPhoneGroom.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ ...NAV_BTN, background: '#25D366', color: '#fff', padding: '10px 16px', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', transition: 'opacity .18s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '.82'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.opacity = '1'}><WaIcon/> WhatsApp</a>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Accommodation / Transport badges */}
        {(isAccommodationActive || isTransportActive) && (
          <div style={{ ...a(.38), display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 40 }}>
            {isAccommodationActive && (<div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 18px', background: '#fff', border: `1px solid ${RULE}`, fontFamily: "'DM Sans',sans-serif", fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: MID }}><span>🏠</span> Cazare disponibilă</div>)}
            {isTransportActive && (<div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 18px', background: '#fff', border: `1px solid ${RULE}`, fontFamily: "'DM Sans',sans-serif", fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: MID }}><span>🚌</span> Transport disponibil</div>)}
          </div>
        )}
            ...
            <div style={{ background: DARK, padding: 'clamp(28px,4vw,44px)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div aria-hidden style={{ position: 'absolute', right: 24, top: '50%', transform: 'translateY(-50%)', opacity: .04, pointerEvents: 'none' }}>
                <svg viewBox="0 0 80 80" fill="white" style={{ width: 'clamp(60px,12vw,110px)', height: 'clamp(60px,12vw,110px)' }}>
                  <rect x="4" y="22" width="72" height="50" rx="4"/><path d="M24 22L28 12h24l4 10"/><circle cx="40" cy="47" r="14"/><circle cx="40" cy="47" r="8"/><circle cx="62" cy="32" r="4"/>
                </svg>
              </div>
              <div style={{ position: 'relative', zIndex: 1, maxWidth: 440 }}>
                ...
                <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(20px,2.8vw,28px)', fontStyle: 'italic', fontWeight: 400, color: '#fff', marginBottom: 12, lineHeight: 1.2 }}>Împărtășiți momentele<br/>cu noi</h3>
                ...
                <button onClick={() => setGalleryModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '12px 24px', background: ACCENT, color: '#fff', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 12, letterSpacing: '.18em', textTransform: 'uppercase', fontWeight: 500, transition: 'opacity .18s' }} onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.opacity = '.82'} onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.opacity = '1'}>
        ...
        {isMenuActive && menuDetails?.categories && (
          <div style={{ ...a(.45), width: '100%', marginBottom: 40 }}>
            <button onClick={() => setMenuModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: 'clamp(13px,1.8vw,16px) clamp(24px,3vw,36px)', background: '#fff', color: DARK, border: `1px solid ${RULE}`, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(11px,1.3vw,13px)', fontWeight: 500, letterSpacing: '.24em', textTransform: 'uppercase', transition: 'all .2s', boxShadow: '0 2px 8px rgba(0,0,0,.04)' }} onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = DARK; (e.currentTarget as HTMLButtonElement).style.color = '#fff'; (e.currentTarget as HTMLButtonElement).style.borderColor = DARK; }} onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#fff'; (e.currentTarget as HTMLButtonElement).style.color = DARK; (e.currentTarget as HTMLButtonElement).style.borderColor = RULE; }}>
        ...
        <div style={{ ...a(.50), width: '100%', marginBottom: 40 }}>
          ...
          <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(14px,1.7vw,17px)', fontStyle: 'italic', color: MID, marginBottom: 16, lineHeight: 1.7 }}>
            Vă rugăm să confirmați prezența Dvs. până la data evenimentului.
          </p>
          <button onClick={() => setRsvpModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 12, padding: 'clamp(13px,1.8vw,16px) clamp(24px,3vw,36px)', background: DARK, color: '#fff', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(11px,1.3vw,13px)', fontWeight: 500, letterSpacing: '.24em', textTransform: 'uppercase', transition: 'background .2s', position: 'relative', overflow: 'hidden' }} onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.background = ACCENT} onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background = DARK}>
            Confirmă Prezența
        ...
        <div onClick={() => setMenuModal(false)} style={MODAL_OVERLAY}>
          <div onClick={e => e.stopPropagation()} style={{ ...MODAL_BOX, maxWidth: 560 }}>
            <div style={{ marginBottom: 22 }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(22px,3.5vw,30px)', fontStyle: 'italic', fontWeight: 400, color: DARK }}>Meniu Eveniment</h2>
              <div style={{ width: 32, height: 2, background: ACCENT, marginTop: 10 }}/>
            </div>
            {menuDetails.categories.filter((cat: any) => cat.active && cat.items && cat.items.length > 0).map((cat: any, cIdx: number) => (
              <div key={cIdx} style={{ marginBottom: 28 }}>
                <div style={{ borderBottom: `1px solid ${RULE}`, paddingBottom: 8, marginBottom: 14 }}>
                  <h4 style={{ color: DARK, fontSize: 'clamp(12px,1.5vw,14px)', margin: 0, fontWeight: 500, letterSpacing: '1.5px', fontFamily: "'DM Sans',sans-serif", textTransform: 'uppercase' }}><span style={{ marginRight: 8 }}>{cat.emoji}</span>{cat.label}</h4>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {cat.items.map((item: any, iIdx: number) => (
                    <div key={iIdx}>
                      <div style={{ color: DARK, fontSize: 'clamp(14px,1.8vw,17px)', fontFamily: "'Playfair Display',serif", fontStyle: 'italic' }}>{item.name}</div>
                      {item.description && (<div style={{ fontSize: 'clamp(11px,1.2vw,13px)', color: LIGHT, fontStyle: 'italic', marginTop: 3, fontFamily: "'DM Sans',sans-serif" }}>{item.description}</div>)}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 8 }}>
              <button onClick={() => setMenuModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: LIGHT, letterSpacing: '.16em', textTransform: 'uppercase' }}>Închide</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div onClick={() => setGalleryModal(false)} style={MODAL_OVERLAY}>
          <div onClick={e => e.stopPropagation()} style={{ ...MODAL_BOX, maxWidth: 400, textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
              <div style={{ width: 64, height: 64, background: 'rgba(200,80,58,.08)', border: `2px solid rgba(200,80,58,.2)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>📸</div>
            </div>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, fontStyle: 'italic', fontWeight: 400, color: DARK, marginBottom: 10 }}>Galerie Foto Live</h2>
            <div style={{ width: 32, height: 2, background: ACCENT, margin: '0 auto 14px' }}/>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: MID, marginBottom: 24, lineHeight: 1.85, fontWeight: 300 }}>Împărtășește momentele surprinse de tine cu mirii.<br/>Apasă butonul de mai jos pentru a încărca pozele tale.</p>
            <a href={`/invitatie/minimal/${slug}/upload`} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '13px 32px', background: DARK, color: '#fff', textDecoration: 'none', fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 500, letterSpacing: '.18em', textTransform: 'uppercase', marginBottom: 16, transition: 'background .2s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.background = ACCENT} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.background = DARK}>📷 Încarcă Pozele</a>
            <br/>
            <button onClick={() => setGalleryModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: LIGHT, letterSpacing: '.16em', textTransform: 'uppercase' }}>Închide</button>
          </div>
        </div>
      )}
        ...
        <div onClick={() => setRsvpModal(false)} style={MODAL_OVERLAY}>
          <div onClick={e => e.stopPropagation()} style={MODAL_BOX}>
            <div style={{ marginBottom: 22 }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(22px,3.5vw,30px)', fontStyle: 'italic', fontWeight: 400, color: DARK }}>Confirmă Prezența</h2>
              <div style={{ width: 32, height: 2, background: ACCENT, marginTop: 10 }}/>
            </div>
            <MinimalRsvpForm orderId={orderId} showAccommodation={isAccommodationActive} showTransport={isTransportActive}/>
            <div style={{ textAlign: 'center', marginTop: 10 }}>
              <button onClick={() => setRsvpModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: LIGHT, letterSpacing: '.16em', textTransform: 'uppercase' }}>Închide</button>
            </div>
          </div>
        </div>
      )}
...
export default function MinimalClient(props: MinimalInviteClientProps) {
  ...
  return (
    <>
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 clamp(20px,4vw,40px)', background: 'rgba(247,244,240,.97)', borderBottom: '1px solid #E8E4E0', backdropFilter: 'blur(12px)' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: 'clamp(3px,.4vw,5px)', height: '100%', background: ACCENT }}/>
        <a href="/" style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 600, letterSpacing: '.18em', textTransform: 'uppercase', color: DARK, textDecoration: 'none', paddingLeft: 'clamp(8px,1.2vw,14px)', transition: 'color .2s' }} onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = ACCENT} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = DARK}>
