...
import RomanticInviteClient from "./RomanticInviteClient";
...
export async function generateMetadata(
  ...
  const description = `Vă invităm cu dragoste la nunta noastră${dateStr ? `, pe ${dateStr}` : ''}${s.location_name ? `, la ${s.location_name}` : ''}. Confirmați prezența online.`;

  const baseUrl = 'https://www.vibeinvite.ro';
  const url = `${baseUrl}/invitatie/romantic/${params.slug}`;
  const ogImage = `${baseUrl}/api/og/romantic/${params.slug}`;
...
export default async function RomanticInvitationPage({ params }: { params: { slug: string } }) {
  const sql = neon(process.env.DATABASE_URL!);
