...
.io-card-preview.theme-romantic { background: linear-gradient(145deg, #FFF0F5 0%, #FFD6E7 50%, #FFBCD9 100%); }
.io-card-preview.theme-garden   { background: linear-gradient(145deg, #F0FFF4 0%, #C6F6D5 50%, #9AE6B4 100%); }
.io-card-preview.theme-royal    { background: linear-gradient(145deg, #F5F0FF 0%, #D6C6FF 50%, #B794F4 100%); }
.io-card-preview.theme-golden   { background: linear-gradient(145deg, #FFFFF0 0%, #FEFCBF 50%, #FAF089 100%); }
.io-card-preview.theme-ocean    { background: linear-gradient(145deg, #EBF8FF 0%, #BEE3F8 50%, #90CDF4 100%); }
.io-card-preview.theme-rustic   { background: linear-gradient(145deg, #FFF5EB 0%, #FDDCB5 50%, #F6AD55 100%); }
.io-card-preview.theme-noir     { background: linear-gradient(145deg, #2D3748 0%, #1A202C 50%, #171923 100%); }
.io-card-preview.theme-boho     { background: linear-gradient(145deg, #FAF5FF 0%, #E9D8FD 50%, #D6BCFA 100%); }
.io-card-preview.theme-minimal  { background: linear-gradient(145deg, #F7FAFC 0%, #EDF2F7 50%, #E2E8F0 100%); }
.io-card-preview.theme-floral   { background: linear-gradient(145deg, #FFF5F7 0%, #FED7E2 50%, #FBB6CE 100%); }
.io-card-preview.theme-vintage  { background: linear-gradient(145deg, #FFFFF0 0%, #F6E05E 30%, #ECC94B 100%); }
.io-card-preview.theme-luxury   { background: linear-gradient(145deg, #1A1208 0%, #2D1F0E 50%, #3D2A12 100%); }
...
.io-prev-ring  { width: 80px; height: 80px; border-radius: 50%; border: 1px solid currentColor; opacity: .2; position: absolute; animation: io-spin 30s linear infinite; }
.io-prev-ring2 { width: 120px; height: 120px; border-radius: 50%; border: 1px dashed currentColor; opacity: .12; position: absolute; animation: io-spin 50s linear infinite reverse; }
...
.io-prev-title-mock { font-family: 'Cormorant Garamond', serif; font-size: 15px; font-weight: 400; letter-spacing: .08em; text-align: center; z-index: 2; line-height: 1.5; opacity: .55; }
.io-prev-date-mock  { font-size: 10px; letter-spacing: .14em; text-transform: uppercase; z-index: 2; opacity: .4; font-weight: 500; }

.io-locked-overlay {
  position: absolute; inset: 0; z-index: 10;
  background: rgba(253,250,246,.72); backdrop-filter: blur(6px);
  display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px;
}
.io-lock-badge { background: #1A1208; color: #fff; border-radius: 100px; padding: 7px 20px; font-size: 11px; font-weight: 600; letter-spacing: .08em; text-transform: uppercase; }
...
.io-card-tag  { display: inline-flex; align-items: center; gap: 5px; font-size: 10px; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: #FF6B00; background: #FFF4ED; border-radius: 100px; padding: 3px 10px; margin-bottom: 10px; }
...
.io-coming-soon { text-align: center; margin: 56px 0 0; opacity: 0; animation: io-up .7s ease .5s forwards; }
.io-cs-card {
...
const CARDS: Card[] = [
  { id: 1,  name: 'Rosa Eternă',     desc: 'Floral romantic cu accente roz și typography clasică.',          theme: 'theme-romantic', icon: '🌹', category: 'Nuntă',     color: '#D53F8C', available: true,  slug: null },
  { id: 2,  name: 'Grădina Secretă', desc: 'Verde botanical, elegant și plin de prospețime.',               theme: 'theme-garden',   icon: '🌿', category: 'Nuntă',     color: '#276749', available: true,  slug: null },
  { id: 3,  name: 'Regală',          desc: 'Violet regal, auriu și typography grandioasă.',                  theme: 'theme-royal',    icon: '👑', category: 'Corporate', color: '#553C9A', available: false, slug: null },
  { id: 4,  name: 'Aur & Lumină',    desc: 'Nuanțe calde de aur, rafinament clasic.',                       theme: 'theme-golden',   icon: '✨', category: 'Nuntă',     color: '#B7791F', available: true,  slug: 'invitatie-nunta-pdf-auriu' },
  { id: 5,  name: 'Valuri Albastre', desc: 'Ocean bleu, proaspăt și modern — perfect pentru vară.',         theme: 'theme-ocean',    icon: '🌊', category: 'Botez',     color: '#2C7A7B', available: true,  slug: null },
  { id: 6,  name: 'Rustic Cald',     desc: 'Tonuri de portocaliu și teracotă, atmosferă de toamnă.',        theme: 'theme-rustic',   icon: '🍂', category: 'Nuntă',     color: '#C05621', available: true,  slug: null },
  { id: 7,  name: 'Noir Elegant',    desc: 'Dark mode luxuriant — sofisticat și memorabil.',                 theme: 'theme-noir',     icon: '🖤', category: 'Corporate', color: '#E2E8F0', available: false, slug: null },
  { id: 8,  name: 'Boho Chic',       desc: 'Liber, creativ, cu accente de mov pastelat și flori.',          theme: 'theme-boho',     icon: '🌸', category: 'Petrecere', color: '#6B46C1', available: true,  slug: null },
  { id: 9,  name: 'Pure Minimal',    desc: 'Curățenie și simplitate — pentru cei care apreciază mai puțin.',theme: 'theme-minimal',  icon: '◇',  category: 'Corporate', color: '#4A5568', available: false, slug: null },
  { id: 10, name: 'Flori de Cireș',  desc: 'Sakura și romantism japonez într-un design delicat.',           theme: 'theme-floral',   icon: '🌸', category: 'Botez',     color: '#B83280', available: false, slug: null },
  { id: 11, name: 'Vintage 1920',    desc: 'Art deco, aur și nostalgie — o eră de neuitat.',                theme: 'theme-vintage',  icon: '🥂', category: 'Nuntă',     color: '#744210', available: false, slug: null },
  { id: 12, name: 'Luxury Black',    desc: 'Ultra-premium, negru și aur — pentru evenimentul anului.',      theme: 'theme-luxury',   icon: '💎', category: 'Corporate', color: '#F6E05E', available: false, slug: null },
]
        ...
        <div className="io-inner">
          ...
          <div className="io-grid" role="list">
            {filtered.map((card, idx) => {
                  ...
                  <div className="io-card-body">
                    ...
                    <div className="io-card-footer">
                      ...
                      {card.available && card.slug ? (
                        ...
                        <span className="io-card-btn-ghost" aria-label="Indisponibil momentan">În curând</span>
                      )}
                    </div>
                  </div>
                </>
              ...
              return card.available && card.slug ? (
                <Link
                  key={card.id}
                  href={`/${card.slug}`}
                  className="io-card"
